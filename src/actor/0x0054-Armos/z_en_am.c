...
static ColliderCylinderInit sBlockCylinderInit = {
    {
        COLTYPE_METAL,
        AT_NONE,
        AC_ON | AC_HARD | (AC_TYPE_PLAYER | AC_TYPE_OTHER),
        OC1_NONE,
        OC2_NONE,
        COLSHAPE_CYLINDER,
    },
...
static DamageTable sDamageTable = {
    ...
    /* Normal arrow  */ DMG_ENTRY(1, AM_DMGEFF_KILL),
    /* Hammer swing  */ DMG_ENTRY(4, AM_DMGEFF_KILL),
    /* Hookshot      */ DMG_ENTRY(0, AM_DMGEFF_NONE),
    /* Kokiri sword  */ DMG_ENTRY(1, AM_DMGEFF_NONE),
    /* Master sword  */ DMG_ENTRY(0, AM_DMGEFF_NONE),
    /* Giant's Knife */ DMG_ENTRY(4, AM_DMGEFF_KILL),
    /* Fire arrow    */ DMG_ENTRY(1, AM_DMGEFF_KILL),
    /* Ice arrow     */ DMG_ENTRY(0, AM_DMGEFF_ICE),
    ...
    /* Master jump   */ DMG_ENTRY(2, AM_DMGEFF_KILL),
    /* Unknown 1     */ DMG_ENTRY(0, AM_DMGEFF_NONE),
    /* Unblockable   */ DMG_ENTRY(4, AM_DMGEFF_KILL),
    /* Hammer jump   */ DMG_ENTRY(8, AM_DMGEFF_KILL),
...
s32 EnAm_CanMove(EnAm* this, PlayState* play, f32 distance, s16 yaw) {
    ...
    if (!ret && (this->dyna.actor.floorHeight > (this->lastPos.y - 109.0f))) {
        ret = true;
    }
...
void EnAm_Destroy(Actor* thisx, PlayState* play) {
    ...
    if(this->dyna.actor.params != ARMOS_STATUE){
        Collider_DestroyQuad(play, &this->hitCollider);
    }
...
void EnAm_SetupCooldown(EnAm* this) {
    Animation_PlayLoopSetSpeed(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->unk_258 = 3;
    this->cooldownTimer = (s16)(20.0f * Rand_ZeroOne()); //40
    this->behavior = AM_BEHAVIOR_AGGRO;
    this->dyna.actor.speedXZ = 0.0f;
    this->dyna.actor.world.rot.y = this->dyna.actor.shape.rot.y;
...
void EnAm_MoveToHome(EnAm* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame == 8.0f) {
        ...
        if (!(this->dyna.actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
            ...
            Math_SmoothStepToS(&this->dyna.actor.world.rot.y, this->dyna.actor.yawTowardsPlayer, 1, 0xBB8, 0);
            ...
            if (Actor_WorldDistXYZToPoint(&this->dyna.actor, &this->dyna.actor.home.pos) < 80.0f) {
                /*EnAm_SetupSleep(this);*/ EnAm_SetupRotateToInit(this);
            }
        }
    }
...
void EnAm_Lunge(EnAm* this, PlayState* play) {
    if (this->deathTimer < 52) {
        if (this->skelAnime.curFrame == 8.0f) {
            this->lastPos = this->dyna.actor.world.pos;
            this->dyna.actor.velocity.y = 12.0f;//JUMP
            ...
            if (!(this->dyna.actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
                ...
                if (((this->attackTimer <= 0)) &&(this->deathTimer == 0)) {
                        EnAm_SetupSleep(this);
                }
            }
        }
        ...
        if ((this->unk_258 == 0) && (this->deathTimer == 0) && this->dyna.actor.xzDistToPlayer > 120.0f && (this->attackTimer > 0) ) {
...
void EnAm_SetupStunned(EnAm* this, PlayState* play) {
    ...
    if (this->damageEffect == AM_DMGEFF_ICE) {
        this->iceTimer = 50;
        Actor_SetColorFilter(&this->dyna.actor, 0, 120, 0, this->iceTimer * 2);
    }else{
...
void EnAm_Update(Actor* thisx, PlayState* play) {
    ...
    //Debug_Text(255,255,255,1,1,"AttackTimer: %d",this->attackTimer);
    if (this->dyna.actor.params != ARMOS_STATUE) {
    ...
    if (this->dyna.actor.colChkInfo.damageEffect != AM_DMGEFF_MAGIC_FIRE_LIGHT) {
        if (this->attackTimer > 0) {
            if(this->dyna.actor.xzDistToPlayer <= 80.0f){
                this->attackTimer = this->attackTimer - 10;
            }else{
                this->attackTimer--;
            }
        }else{
        ...
        Actor_MoveForward(&this->dyna.actor);
        Actor_UpdateBgCheckInfo(play, &this->dyna.actor, 20.0f, 15.0f, 80.0f,
    ...
    if (this->dyna.actor.params != ARMOS_STATUE) {
        ...
        if ((this->behavior >= 4) && (this->unk_264 > 0)) {
            if (!(this->hitCollider.base.atFlags & AT_BOUNCED)) {
                ...
                this->attackTimer = this->attackTimer - 50;
                this->hitCollider.base.atFlags &= ~(AT_HIT | AT_BOUNCED);
...
void EnAm_Draw(Actor* thisx, PlayState* play) {
    ...
    gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 255, this->textureBlend);
