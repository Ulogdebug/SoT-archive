...
s16 EnGo2_GetStateGoronMonitor(PlayState* play, EnGo2* this);
u16 EnGo2_GetTextIdGoronMonitor(PlayState* play, EnGo2* this);
...
u16 EnGo2_GetTextIdGoronCityRollingBig(PlayState* play, EnGo2* this) {
    if (GET_INFTABLE(INFTABLE_11E)) {
        return 0x3013;
    } else if (CUR_CAPACITY(UPG_BOMB_BAG) >= 20 && this->waypoint > 5 && this->waypoint < 14) {
        return 0x3012;
    } else {
        return 0x3011;
    }
...
u16 EnGo2_GetTextIdGoronMonitor(PlayState* play, EnGo2* this) {
    if (!GET_INFTABLE(INFTABLE_F8)) {
        return 0x0182;
    } else if (gSaveContext.rupees >= 99 && (CustomFlag_Get(FREE_SHACKLES) == 0) ) {
        return 0x209D;
...
s16 EnGo2_GetStateGoronMonitor(PlayState* play, EnGo2* this) {
    ...
    switch (Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_CLOSING:
            if (this->actor.textId == 0x0182) {
                SET_INFTABLE(INFTABLE_F8);
            }
            return 2;
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x209D) {
                    this->actionFunc = EnGo2_SetupGetItem;
                    EnGo2_GetItem(this, play, GI_BOOTS_IRON);
                    //Rupees_ChangeBy(-99);
                    Message_CloseTextbox(play);
                    CustomFlag_Set(LOCKED_SHACKLES,0);
                    CustomFlag_Set(FREE_SHACKLES,1);
                    return 2;
                } else {
                    return 2;
                }
...
s16 EnGo2_GetStateGoronNayruLove(PlayState* play, EnGo2* this) {

    switch (Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_CLOSING:
            if(!CustomFlag_Get(0x20))
                CustomFlag_Set(0x20,1);
            return 2;
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x7180) {
                    if(!CustomFlag_Get(0x20))
                        CustomFlag_Set(0x20,1);
                    this->actionFunc = EnGo2_SetupGetItem;
                    EnGo2_GetItem(this, play, GI_NAYRUS_LOVE);
                    Message_CloseTextbox(play);
                    return 2;
                } else {
                    return 2;
...
u16 EnGo2_GetTextIdGoronNayruLove(PlayState* play, EnGo2* this) {
    Player* player = GET_PLAYER(play);
    if(INV_CONTENT(ITEM_NAYRUS_LOVE) == ITEM_NAYRUS_LOVE){
        return 0x719B;
    }
    if(player->currentBoots == PLAYER_BOOTS_IRON && !CustomFlag_Get(0x20)){
         return 0X0188;
    }else if (player->currentBoots == PLAYER_BOOTS_IRON && CustomFlag_Get(0x20)){
        return 0X0187;
        ...
        switch (this->actor.params & 0x1F) {
            ...
            case GORON_DMT_BOMB_FLOWER:
                if(play->sceneId != 0x5){
                    ...
                    return EnGo2_GetTextIdGoronMonitor(play, this);
                }
            ...
            case GORON_DMT_DC_ENTRANCE:
                if(play->sceneId != 0x5){
                    ...
                    return EnGo2_GetTextIdGoronNayruLove(play, this);
                }
        ...
        case GORON_DMT_BOMB_FLOWER:
            if(play->sceneId != 0x5){
                return EnGo2_GetStateGoronDmtBombFlower(play, this);
                 ...
                 return EnGo2_GetStateGoronMonitor(play, this);
            }
        case GORON_DMT_ROLLING_SMALL:
        ...
        case GORON_DMT_DC_ENTRANCE:
            if(play->sceneId != 0x5){
                return EnGo2_GetStateGoronDmtDcEntrance(play, this);
                 ...
                 return EnGo2_GetStateGoronNayruLove(play, this);
            }        
        case GORON_CITY_ENTRANCE:
        ...
        if ((this->actionFunc != EnGo2_SlowRolling) && (this->actionFunc != EnGo2_ReverseRolling) &&
            ...
            if (this->collider.base.acFlags & AC_HIT) {
                if(play->sceneId != 0x5){
                    ...
                    Message_StartTextbox(play, 0x01A6, NULL);
                }
                return true;
            }
...
void EnGo2_BiggoronSetTextId(EnGo2* this, PlayState* play, Player* player) {
    ...
    if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        if (gSaveContext.bgsFlag) { //already get Bg sowrd
            if (func_8002F368(play) == EXCH_ITEM_CLAIM_CHECK) {
...
void EnGo2_Init(Actor* thisx, PlayState* play) {
    ...
    this->unk_26E = 1;
    this->path = Path_GetByIndex(play, (this->actor.params >> 8) & 0xFF, 0x1F);
    switch (this->actor.params & 0x1F) {
        case GORON_CITY_ENTRANCE:
        case GORON_CITY_ISLAND:
        case GORON_CITY_LOWEST_FLOOR:
    ...
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_DONE) && Message_ShouldAdvance(play)) {
        ...
        switch (this->getItemId) {
            ...
            case GI_TUNIC_GORON:
                ...
                return;
            case GI_SWORD_BGS:
                Inventory_DeleteItem(ITEM_CLAIM_CHECK,SLOT(ITEM_CLAIM_CHECK));
