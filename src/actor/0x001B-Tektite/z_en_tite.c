...
void EnTite_Init(Actor* thisx, PlayState* play) {
    ...
    thisx->targetMode = 3;//original was 3
    ...
    if (SkelAnime_Update(&this->skelAnime) != 0) {
        ...
        switch (this->vAttackState) {
            case TEKTITE_BEGIN_LUNGE:
                ...
                else if((this->actor.xzDistToPlayer >= 30.0f) && (this->actor.xzDistToPlayer < 150.0f)) {
                    this->actor.speedXZ = 4.0f;
                    this->actor.velocity.y = 12.0f;//original was 8.0f
                }
                else if(this->actor.xzDistToPlayer < 30.0f)
                {
                    this->actor.speedXZ = 4.0f;
                    this->actor.velocity.y = 4.0f;//original was 8.0f
                }
                break;
            case TEKTITE_MID_LUNGE:
                ...
                if ((this->actor.xzDistToPlayer <= 5.0f) && (this->actor.yDistToPlayer >= 5.0f)) {
...
void EnTite_TurnTowardPlayer(EnTite* this, PlayState* play) {
    ...
    if (angleToPlayer > 0) {
        turnVelocity = (angleToPlayer / 42.0f) + 10.0f;//original was +10.0f
