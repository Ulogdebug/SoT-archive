...
void EnBoom_Bomb(Actor* thisx, PlayState* play);
void EnBoom_SetupFly(EnBoom* this, PlayState* play) ;
...
void EnBoom_SetupFly(EnBoom* this, PlayState* play) {
    if( ((this->actor.params >> 8) & 0xFF) == 0x3F){
        this->bombFlag = 1;
    }else{
        this->bombFlag = 0;
    }
...
void EnBoom_Fly(EnBoom* this, PlayState* play) {
    ...
    if (target != NULL ) { //Won't pick up pice of heart or small key.
    ...
    collided = !!(collided);
    if (collided) {
        if ((((this->collider.base.at->id == ACTOR_EN_ITEM00) && ((this->collider.base.at->params & 0xFF) != 0x06 ) && ((this->collider.base.at->params & 0xFF) != 0x11 )) || (this->collider.base.at->id == ACTOR_EN_SI))) {
    ...
    if (DECR(this->returnTimer) == 0 || (!(player->stateFlags1 & PLAYER_STATE1_25))) {
        ...
        if (distFromLink < 40.0f|| (!(player->stateFlags1 & PLAYER_STATE1_25))) {
            target = this->grabbed;
            if (target != NULL) {
            ...
            if(this->isExploded == 0 && this->bombFlag == 1 ){
                AMMO(ITEM_BOMB)++;
            }
...
void EnBoom_Draw(Actor* thisx, PlayState* play) {
    ...
    //Debug_Text(255, 0, 0, 1, 1, "! %x", this->returnTimer);
