...
static u8 sOsAnimeLookup[13][5] = {
    ...
    /* ENKO_TYPE_CHILD_3    */ { ENKO_ANIM_0, ENKO_ANIM_0, ENKO_ANIM_0, ENKO_ANIM_0/*ENKO_ANIM_17*/, ENKO_ANIM_0 },//Boxing Boy
    /* ENKO_TYPE_CHILD_4    */ { ENKO_ANIM_20, ENKO_ANIM_20, ENKO_ANIM_20, ENKO_ANIM_20/*ENKO_ANIM_10*/, ENKO_ANIM_20 },//Back Flipping
...
static EnKoInteractInfo sInteractInfo[] = {
    /* ENKO_TYPE_CHILD_0    */ { 6, 30.0f, 180.0f }, //the last decides appear distance
    ...
    /* ENKO_TYPE_CHILD_5    */ { 1, 30.0f, 600.0f },//240.0f
    /* ENKO_TYPE_CHILD_6    */ { 6, 30.0f, 180.0f },
    /* ENKO_TYPE_CHILD_7    */ { 6, 30.0f, 180.0f },
    /* ENKO_TYPE_CHILD_8    */ { 6, 30.0f, 600.0f },//180.0f
...
u16 func_80A96FD0(PlayState* play, Actor* thisx) {
    ...
    switch (ENKO_TYPE) {
        ...
        case ENKO_TYPE_CHILD_2:
            ...
            return 0x1007;
        case ENKO_TYPE_CHILD_3://chest boy
            if (CustomFlag_Get(0x6)) {
                return 0x1021;
            }
             return 0x105F;
...
u16 func_80A97338(PlayState* play, Actor* thisx) {
    ...
    switch (ENKO_TYPE) {
        ...
        case ENKO_TYPE_CHILD_0:
            ...
            //     return 0x1056;
            // }
            if(play->sceneId == 0x6A){
                if(Flags_GetSwitch(play,0x7)){// the cell door opened
                    if(!CustomFlag_Get(0x6)){
                        CustomFlag_Set(0x6,1);
                    }
                    return 0x1056;
                }else{
                    return 0x1055;
        ...
        case ENKO_TYPE_CHILD_1: //warn you IK is in lost woods
            if (CustomFlag_Get(0x6)) {
                return 0x105A;
            }
           if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
            ...
            return 0x1073;
        case ENKO_TYPE_CHILD_2: // guard the entrance
        ...
        case ENKO_TYPE_CHILD_3: //chest boy
            if (CustomFlag_Get(0x6)) {
             ...
             return 0x105F;
        case ENKO_TYPE_CHILD_4: //boxing boy
           if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
                return 0x1077;
            }
            return 0x1076;
        case ENKO_TYPE_CHILD_5: //the sitting girl on top of the tree
            if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
                return 0x1079;
            }
            return 0x1057;
        ...
        case ENKO_TYPE_CHILD_7:
            ...
            return 0x104E;
        case ENKO_TYPE_CHILD_8://relate to the factory
            if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
                return 0x107A;
            }
            ...
            // }
            return 0x104F;
        case ENKO_TYPE_CHILD_9: //excited about a new member
            if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
                return 0x107B;
            }
            return 0x1051;
        case ENKO_TYPE_CHILD_10:
         /*   if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
            ...
            return 0x1052; //say you will lose in lost woods

        case ENKO_TYPE_CHILD_11://says he envy you as you're adult ,sitting
            if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST)) {
                return 0x107C;
            }
...
s32 EnKo_GetForestQuestState(EnKo* this) {
    ...
    // }
    return ENKO_FQS_ADULT_ENEMY;//result;
}
...
void func_80A9877C(EnKo* this, PlayState* play) {
    ...
    if (func_800343CC(play, &this->actor, &this->unk_1E8.unk_00, this->lookDist, func_80A97610, func_80A97738) &&
        ENKO_TYPE == ENKO_TYPE_CHILD_FADO && play->sceneId == SCENE_SPOT10) { // Lost Woods
...
s32 EnKo_GetForestQuestState2(EnKo* this) {
    if (LINK_IS_ADULT) {
        return ENKO_FQS_ADULT_ENEMY;//CHECK_QUEST_ITEM(QUEST_MEDALLION_FOREST) ? ENKO_FQS_ADULT_SAVED : ENKO_FQS_ADULT_ENEMY;
    }
    if (CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD)) {
...
void func_80A98DB4(EnKo* this, PlayState* play) {
    ...
    if (play->csCtx.state != 0 || gDbgCamEnabled != 0) {
        dist = Math_Vec3f_DistXYZ(&this->actor.world.pos, &play->view.eye) * 0.25f; // * 0.25f
    } else {
        dist = this->actor.xzDistToPlayer;
    }

    Math_SmoothStepToF(&this->modelAlpha, (this->appearDist < dist) ? 0.0f : 255.0f, 0.3f, 40.0f, 1.0f); //orignal dist = 255.0f
...
void EnKo_Init(Actor* thisx, PlayState* play) {
    ...
    if(ENKO_TYPE == ENKO_TYPE_CHILD_0 && !CustomFlag_Get(0x6) && play->sceneId == 0x4C){
        Actor_Kill(thisx);
    }
    if(ENKO_TYPE == ENKO_TYPE_CHILD_0 && CustomFlag_Get(0x6) && play->sceneId == 0x6A){
        Actor_Kill(thisx);
    }
...
void func_80A99048(EnKo* this, PlayState* play) {
    if (EnKo_IsOsAnimeLoaded(this, play) && EnKo_AreObjectsLoaded(this, play)) {
        ...
        Animation_ChangeByInfo(&this->skelAnime, sAnimationInfo, sOsAnimeLookup[ENKO_TYPE][this->forestQuestState]); //Wow
        ...
        if (ENKO_TYPE == ENKO_TYPE_CHILD_3) {
            if (!CustomFlag_Get(0x6)/*!CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD)*/) {
