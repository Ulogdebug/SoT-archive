...
static EnHorseSpawnpoint sHorseSpawns[] = {
    // Wasteland
    { 0x57, -1346, -257, 1038, 0 },
    { 0x57, -427, -262, 1435, 0 },
    { 0x57, -3092, -1033, 3527, 0 },
    { 0x57, 986, -11, 1715, 0 },
    { 0x57, 1929, 11, 2484, 0 },
    { 0x57, 1208, 65, 3681, 0 },
    { 0x57, -352, 38, 3176, 0 },
    ...
    { 0x3F, -3278, 33, -829, 0 },
    { 0x3F, -3555, 33, -2085, 0 },
    { 0x3F, -2342, 209, -1202, 0 },
    { 0x3F, -1995, 198, -1161, 0 },
    { 0x3F, -2808, 101, -1072, 0 },
    { 0x3F, -3548, 33, -2014, 0 },
    { 0x3F, -3942, 33, -2702, 0 },

    // Border Desert
    { 0x6B, 2121, -432, -740, 0},
    { 0x6B, 1896, -456, -1051, 0 },
    { 0x6B, 2747, -589, -1607, 0 },
    { 0x6B, 2648, -543, -1043, 0 },
    { 0x6B, 3680, -492, -482, 0 },
    { 0x6B, 4500, -520, -1337, 0 },
    { 0x6B, 6715, -325, -1302, 0 },
    { 0x6B, 5455, -519, -2031, 0 },
    { 0x6B, 6771, -306, -1177, 0 },

    { 0x6B, 6966, -307, 189, 0},
    { 0x6B, 8974, -176, 173, 0 },
    { 0x6B, 10894, -220, 829, 0 },
    { 0x6B, 13270, -303, 1514, 0 },
    { 0x6B, 14995, -397, 2412, 0 },
    { 0x6B, 15869, -416, 2821, 0 },
    { 0x6B, 10724, -256, 3372, 0 },
    { 0x6B, 12653, -151, -688, 0 },
    { 0x6B, 10322, -51, -1885, 0 },
    { 0x6B, 7457, -203, 1802, 0 },
    { 0x6B, 7465, -190, -2292, 0 },

    { 0x6B, 9464, -178, 1332, 0},
    { 0x6B, 11493, -288, 2354, 0 },
    { 0x6B, 13993, -429, 2943, 0 },
    { 0x6B, 14370, -253, 857, 0 },
    { 0x6B, 11084, -72, -1606, 0 },
    { 0x6B, 9003, -76, -2014, 0 },
    { 0x6B, 7810, -225, 374, 0 },
    { 0x6B, 8037, -180, 2695, 0 },
    { 0x6B, 9914, -211, 4092, 0 },
    { 0x6B, 8656, -181, 19, 0 },
    ...
    // HorsebackArchery
    { 0x5D, 3327, 1413, -1890, 0},
    { 0x5D, 3344, 1413, -3063, 0},
    { 0x5D, 3388, 1413, -4419, 0},
    { 0x5D, 3141, 1413, -696, 0},
    { 0x5D, 3371, 1413, 152, 0},
...
s32 EnHorse_Spawn(EnHorse* this, PlayState* play) {
    ...
    Player* player;
    Vec3f spawnPos;
    //Debug_Text(255,255,255,1,5," Oh no");
    for (i = 0; i < 83; i++) {
        // if(sHorseSpawns[i].scene == 0xFF){
        //     return 0;
        // }
...
void EnHorse_Init(Actor* thisx, PlayState* play2) {
    ...
    //Flags_SetEventChkInf(EVENTCHKINF_18);
    if (!Flags_GetEventChkInf(EVENTCHKINF_18) && play->sceneId == 0x6B) {
        Flags_SetEventChkInf(EVENTCHKINF_18);
        Audio_PlayActorSfx2(&this->actor, NA_SE_VO_NA_HELLO_2); 
        Message_StartTextbox(play, 0x70A5, NULL);                    

    }else if(Flags_GetEventChkInf(EVENTCHKINF_18) && play->sceneId == 0xB){
    ...
    // if ((play->sceneId == SCENE_SPOT20) && (gSaveContext.sceneLayer < 4)) {
    ...
    } else if (play->sceneId == 0xB && !Flags_GetEventChkInf(EVENTCHKINF_18) /*&& !DREG(1)*/) { //EVENTCHKINF_18 = get horse flag
        EnHorse_InitFleePlayer(this);
    } else {
        if (play->sceneId == SCENE_SOUKO) {
            EnHorse_ResetIdleAnimation(this);
        } else if (play->sceneId == 0xB && this->type == HORSE_HNI) {
            EnHorse_ResetIdleAnimation(this);
        }else if (play->sceneId == 0x5D && this->type == HORSE_HNI) {
            EnHorse_ResetIdleAnimation(this);
        }else{
...
void EnHorse_FleePlayer(EnHorse* this, PlayState* play) {
    ...
    if (DREG(53) || this->type == HORSE_HNI) { //play epona's song
        EnHorse_StartIdleRidable(this);
        Audio_PlaySfxGeneral(NA_SE_EV_HORSE_NEIGH, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                               &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        Message_StartTextbox(play, 0x2026, NULL);
        Audio_PlayActorSfx2(&this->actor, NA_SE_VO_NA_HELLO_2); 
    }
...
void EnHorse_Draw(Actor* thisx, PlayState* play) {
   ...
   /* GfxPrint printer;
    ...
    GfxPrint_SetPos(&printer, 1, 1);
        GfxPrint_Printf(&printer, "EponaPos X=%.1f Y=%.1f Z=%.1f", this->actor.world.pos.x, this->actor.world.pos.y, this->actor.world.pos.z);
