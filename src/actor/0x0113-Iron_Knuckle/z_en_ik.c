...
void EnIk_WakeUpJudge(EnIk* this, PlayState* play);
void EnIk_AttackJudge(EnIk* this, PlayState* play);
void EnIk_Lightning(EnIk* this, PlayState* play,s16 Color);
void EnIk_SpawnDirtEffect(EnIk* this, PlayState* play);
void En_IkMove(EnIk* this);
void EnIk_MoveAttackJudge(EnIk* this, PlayState* play);
void EnIk_SetupVerticalCut(EnIk* this);
void EnIk_VerticalCut(EnIk* this, PlayState* play);
void func_80A7506C(EnIk* this);
void func_80A7510C(EnIk* this, PlayState* play);
void EnIk_SetupCrossCut(EnIk* this);
void EnIk_CrossCut(EnIk* this, PlayState* play);
void func_80A753D0(EnIk* this);
void EnIk_SetupArmorFall(EnIk* this, PlayState* play);
void EnIk_NoArmorTakeDamage(EnIk* this, PlayState* play);
void EnIk_SetupNoArmorTakeDamage(EnIk* this);
void func_80A7545C(EnIk* this, PlayState* play);
void EnIk_SetupInsCrossCut(EnIk* this);
void EnIk_SetupInsCricleCut(EnIk* this);
void EnIk_InsCricleCut(EnIk* this, PlayState* play);
void EnIk_InsCrossCut(EnIk* this, PlayState* play);
void EnIk_SetupBlock(EnIk* this);
void EnIk_Block(EnIk* this, PlayState* play);
void EnIk_ArmorFall(EnIk* this, PlayState* play);
void EnIk_Dead(EnIk* this, PlayState* play);
void EnIk_CutChoose(EnIk* this);
void EnIk_SetupMoveAttackJudge(EnIk* this);
...
void EnIk_CheckHealth(EnIk* this, PlayState* play);

static ColliderCylinderInit sCylinderInit = { //bodyCollider
    {
        COLTYPE_NONE,
        AT_NONE,
...
static ColliderTrisInit sTrisInit = {   //shieldCollider
    {
        COLTYPE_METAL,
        AT_NONE,
...
static ColliderQuadInit sQuadInit = {//Axe Collider
    {
        COLTYPE_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
static DamageTable sDamageTable = {
    ...
    /* Giant spin    */ DMG_ENTRY(2, 0xE),
    /* Master spin   */ DMG_ENTRY(1, 0xE),
...
void EnIk_Destroy(Actor* thisx, PlayState* play) {
    ...
    // Collider_DestroyCylinder(play, &this->lightningCollider);
...
void EnIk_Init2(Actor* thisx, PlayState* play) {
    EnIk* this = (EnIk*)thisx;
    s32 pad;
    ...
    if (this->switchFlags != 0x3F) {
        if (Flags_GetSwitch(play, this->switchFlags)) {
           Actor_Kill(thisx);
           return;
        }
    } 
    if (CustomFlag_Get(0x5)) {
        Actor_Kill(thisx);
        return;
    }
}
...
s32 EnIk_BlockJudge(EnIk* this, PlayState* play) {
    if (
...
Actor* func_80A74674(PlayState* play, Actor* actor) {
    ...
    while (prop != NULL) {
        if ((prop == actor) || (prop->id != ACTOR_BG_JYA_IRONOBJ)|| (prop->id != ACTOR_EN_RR)) {
            prop = prop->next;
            continue;
...
void func_80A74714(EnIk* this) {
    ...
    EnIk_SetupAction(this, EnIk_WakeUpJudge);
}

void EnIk_WakeUpJudge(EnIk* this, PlayState* play) {
    Vec3f sp24;

    if (((this->actor.xzDistToPlayer < 600.0f) && (this->actor.yDistToPlayer < 80.0f))||(this->bodyCollider.base.acFlags & AC_HIT)) {
...
void EnIk_SetupAttackJudge(EnIk* this) {
    ...
    EnIk_CutChoose(this);
    EnIk_SetupAction(this, EnIk_AttackJudge);
}

void EnIk_AttackJudge(EnIk* this, PlayState* play) {
    s32 phi_a0 = (this->armorDown == 0) ? 0xAAA : 0x3FFC;
    s16 yawDiff = this->actor.yawTowardsPlayer - this->actor.shape.rot.y;

    if (this->actor.speedXZ >= 2.0f && (Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) == true)) {
        if (this->cutAction == VERTICALCUT) {
            EnIk_Lightning(this, play, Yellow);
        }
        else {
            EnIk_Lightning(this, play, Red);
    ...
    if ((ABS(yawDiff) <= phi_a0) && (this->actor.xzDistToPlayer < 100.0f) &&
        ...
        if (this->cutAction == VERTICALCUT) {
            EnIk_SetupVerticalCut(this);
        } else {
            EnIk_SetupCrossCut(this);
        }
    }
    ...
    EnIk_BlockJudge(this, play);
    SkelAnime_Update(&this->skelAnime);
}

void EnIk_Lightning(EnIk* this, PlayState* play, s16 Color){
    static Color_RGBA8 primColor = { 255, 255, 255, 255 };
    static Color_RGBA8 Y = { 255, 255, 0, 255 };//VerticalCut
    static Color_RGBA8 R = { 255, 50, 50, 255 };   //CrossCut
    ...
    for (i = 0; i < 4; i++) {
        if (!((this->timer + (i << 1)) % 4)) {
            ...
            effectPos.x = Rand_CenteredFloat(5.0f) + this->axePos.x;
            effectPos.y = (Rand_ZeroOne() * 5.0f) + this->axePos.y;
            effectPos.z = Rand_CenteredFloat(5.0f) + this->axePos.z;
            if (Color == Yellow) {
         
            EffectSsLightning_Spawn(play, &effectPos, &primColor, &Y, effectXZ, effectYaw, 3, random);
            }
            else if (Color == Red) {
                EffectSsLightning_Spawn(play, &effectPos, &primColor, &R, effectXZ, effectYaw, 3, random);
            }
        }
    }
...
void EnIk_SpawnDirtAxeEffect(EnIk* this, PlayState* play) {

    static Vec3f accel = { 0.0f, 2.0f, 5.0f };
    Vec3f pos;
    Vec3f velocity;
...
void En_IkMove(EnIk* this) {
    ...
    if (this->armorDown == false) {
        ...
        if (((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) == true)) && (this->actor.xzDistToPlayer < 200.0f))
        {
            this->skelAnime.playSpeed = 1.5f;
            this->actor.speedXZ = 5.0f;//original was 2.5f
        }
        else {
    ...
    EnIk_SetupMoveAttackJudge(this);
}

void EnIk_SetupMoveAttackJudge(EnIk* this) {
    EnIk_CutChoose(this);
    EnIk_SetupAction(this,EnIk_MoveAttackJudge);
...
void EnIk_MoveAttackJudge(EnIk* this, PlayState* play) {
    ...
    if (func_80A74674(play, &this->actor) != NULL) {
        ...
        if ((!(Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) )) && (this->actor.xzDistToPlayer < 120.0f)
            && (this->actor.xzDistToPlayer > 60.0f)) {
                EnIk_SetupInsCricleCut(this);
        } else if(this->actor.xzDistToPlayer <= 60.0f && (this->unk_300 <=0)) {
            EnIk_SetupInsCricleCut(this);
            this->unk_300 = 20;
    ...
    if ((sp30 == (s16)this->skelAnime.curFrame) || (sp2E == (s16)this->skelAnime.curFrame)) {
        if (this->actor.speedXZ >= 4.0f) {
            EnIk_SpawnDirtEffect(this, play);
        }
...
void EnIk_SetupVerticalCut(EnIk* this) {
    this->axeCollider.info.toucher.damage = 0x10;
    f32 frames = Animation_GetLastFrame(&object_ik_Anim_001C28);

    this->unk_2FF = 1;
    this->unk_2F8 = 6;
    if ((this->actor.speedXZ < 2.5f) && (this->armorDown == false)) {

        this->actor.speedXZ = 2.5f;

    }
    else if ((this->actor.speedXZ < 5.5f) && (this->armorDown == true)) {
        this->actor.speedXZ = 5.5f;
    }
    Animation_Change(&this->skelAnime, &object_ik_Anim_001C28, 1.5f, 0.0f, frames, ANIMMODE_ONCE, -4.0f);
    EnIk_SetupAction(this, EnIk_VerticalCut);
}
...
void EnIk_VerticalCut(EnIk* this, PlayState* play) {
    ...
    Vec3f effAccel = { 0.0f, 0.0f, 0.0f };
    Vec3f effVelocity = { 0.0f, 0.0f, 0.0f };
    Vec3f effectPos = this->axePos;

    EnIk_Lightning(this, play, Yellow);
    Math_SmoothStepToS(&this->actor.speedXZ, 0.0f, 1, 0x2, 0);
    if (this->actor.speedXZ >= 4.0f) {
        EnIk_SpawnDirtEffect(this, play);
    }
    ...
    if(this->skelAnime.curFrame >= 21.0f && this->skelAnime.curFrame < 23.0f) {

        if(Actor_WorldDistXYZToPoint(&player->actor, &this->axePos) <= 90.0f && (ABS(player->actor.world.pos.y - this->axePos.y)<=20.0f) ){
            func_8002F698(play, &this->actor, 3.0f, this->actor.world.rot.y, 0.0f, 3, 0);
            //play->damagePlayer(play, -4);
         }

        EnIk_SpawnDirtAxeEffect(this, play);
...
void func_80A7506C(EnIk* this) {
    ...
    Animation_Change(&this->skelAnime, &object_ik_Anim_0029FC, 5.0f, 0.0f, frames, ANIMMODE_LOOP, -4.0f);
    ...
    f32 frames;
     Player* player = GET_PLAYER(play);
      ...
      /*if (this->skelAnime.curFrame > 12.0f){
       if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)) && (this->actor.xzDistToPlayer < 100.0f))
        {
           EnIk_SetupCrossCut(this);
           return;
        }
       if(!Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) && (this->actor.xzDistToPlayer <= 60.0f)) {
            EnIk_SetupInsCrossCut(this);
            return;
        }
    }*/
    if (this->unk_2F8 == 8) {
         EnIk_SetupAttackJudge(this);
     }
    if (SkelAnime_Update(&this->skelAnime) || (--this->unk_2F9 == 0)) {
        ...
        if (this->armorDown == false) {
            if((!(player->stateFlags1 & PLAYER_STATE1_26))){
                ...
                else if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)) && (this->actor.xzDistToPlayer >= 100.0f)&&(this->actor.xzDistToPlayer < 120.0f))
                {
                    EnIk_SetupVerticalCut(this);
                    return;
                }
                else if (!Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) && (this->actor.xzDistToPlayer <= 100.0f) /*&& (this->actor.xzDistToPlayer > 60)*/) {
                    EnIk_SetupCrossCut(this);
                    return;
                }
            }
            ...
            if((!(player->stateFlags1 & PLAYER_STATE1_26))){
                if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)) && (this->actor.xzDistToPlayer < 150.0f))
                {
                    EnIk_SetupCrossCut(this);
                    return;
                }
                else if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)) && (this->actor.xzDistToPlayer >= 150.0f) && (this->actor.xzDistToPlayer < 180.0f))
                {
                    EnIk_SetupVerticalCut(this);
                    return;
                }
                else if (!Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) && (this->actor.xzDistToPlayer <= 100.0f)&& (this->actor.xzDistToPlayer >40)) {
                    EnIk_SetupCrossCut(this);
                    return;
                } 
                else if (!Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360) && (this->actor.xzDistToPlayer <= 40.0f)) {
         ...
         this->unk_2F8 = 8;
         Animation_Change(&this->skelAnime, &object_ik_Anim_002538, 1.0f, 0.0f, frames, ANIMMODE_ONCE_INTERP, -4.0f);
    }
...
void EnIk_SetupCrossCut(EnIk* this) {
    ...
    this->unk_2F8 = 6;
    this->unk_2FC = 0;
    if ((this->actor.speedXZ < 2.0f) && (this->armorDown == false)) {
       this->actor.speedXZ = 2.0f;
    }
    else if((this->actor.speedXZ < 4.5f) && (this->armorDown == true)) {
        this->actor.speedXZ = 4.5f;
    }
    Animation_Change(&this->skelAnime, &object_ik_Anim_0033C4, 0.0f, 0.0f, frames, ANIMMODE_ONCE_INTERP, -6.0f);
    EnIk_SetupAction(this, EnIk_CrossCut);
}
...
void EnIk_CrossCut(EnIk* this, PlayState* play) {
    ...
    this->unk_300 += 0x282;//0x1C2
    temp_f0 = Math_SinS(this->unk_300);
    this->skelAnime.playSpeed = ABS(temp_f0);
    Math_SmoothStepToS(&this->actor.speedXZ, 0.0f, 1, 0x05, 0);
    ...
    if (((this->skelAnime.curFrame > 1.0f) && (this->skelAnime.curFrame < 9.0f)) ||
        ...
        if ((this->unk_2FC == 0) && (this->armorDown != 0) && (this->skelAnime.curFrame < 10.0f)) {
            Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0x4DC, 0);
            this->actor.shape.rot.y = this->actor.world.rot.y;
        }
        if (this->unk_2FE < 0) {
            Audio_PlayActorSfx2(&this->actor, NA_SE_EN_IRONNACK_SWING_AXE);
    ...
    if((this->skelAnime.curFrame > 18.0f )&& (this->skelAnime.curFrame < 25.0f)) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        if (this->armorDown == false) {
            if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)) && (this->actor.xzDistToPlayer < 110.0f)&& (!(player->stateFlags1 & PLAYER_STATE1_26)))
            {
                EnIk_SetupCrossCut(this);
                return;
            }  
            ...
            if((!(player->stateFlags1 & PLAYER_STATE1_26))){
                if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)) && (this->actor.xzDistToPlayer < 150.0f) && (this->actor.xzDistToPlayer > 60.0f))
                {
                    EnIk_SetupCrossCut(this);
                    return;
                }
...
void EnIk_SetupInsCrossCut(EnIk* this) {
    ...
    EnIk_SetupAction(this, EnIk_InsCrossCut);
}
...
void EnIk_SetupInsCricleCut(EnIk* this) {
    ...
    Animation_Change(&this->skelAnime, &object_ik_Anim_0033C4, 0.8f, 0.0f, frames, ANIMMODE_ONCE_INTERP, -4.0f); //13.0f star
    EnIk_SetupAction(this, EnIk_InsCricleCut);
...
void EnIk_InsCricleCut(EnIk* this, PlayState* play) {
    ...
    EnIk_Lightning(this, play, Red);
            ...
            //Math_SmoothStepToS(&this->actor.world.rot.y, &this->actor.world.rot.y + 0x600, 1, 0xADC, 0);
   

   /* this->actor.shape.rot.y = this->actor.world.rot.y;
    if ((this->skelAnime.curFrame > 1.0f) && (this->skelAnime.curFrame < 15.0f)) {
        if (this->unk_2FE < 0) {
            Audio_PlayActorSfx2(&this->actor, NA_SE_EN_IRONNACK_SWING_AXE);
        }
        this->unk_2FE = 1;
        this->actor.world.rot.y = this->actor.world.rot.y + 0x550;
    }
    else if((this->skelAnime.curFrame >= 15.0f) && (this->skelAnime.curFrame < 18.0f)) {
    ...
    if (this->skelAnime.curFrame >= 17.0f) {
        ...
        //this->actor.world.rot.y = this->actor.world.rot.y + 0x700;
        Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0x4DC, 0);
        this->actor.shape.rot.y = this->actor.world.rot.y;

    }
    else {
        this->actor.world.rot.y = this->actor.world.rot.y + 0x650;
        this->actor.shape.rot.y = this->actor.world.rot.y;
    }
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0xADC, 0);
        this->actor.shape.rot.y = this->actor.world.rot.y;
        func_80A753D0(this);
        EnIk_BlockJudge(this, play);
    }
...
void EnIk_SetupBlock(EnIk* this) {
    ...
    EnIk_SetupAction(this, EnIk_Block);
}
...
void EnIk_SetupNoArmorTakeDamage(EnIk* this) {
    ...
    this->unk_2FE = 0;
    EnIk_SetupAction(this, EnIk_NoArmorTakeDamage);
...
void EnIk_SetupArmorFall(EnIk* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime) && (ABS((s16)(this->actor.yawTowardsPlayer - this->actor.shape.rot.y)) <= 0x4000) ) {
...
void EnIk_ArmorFall(EnIk* this, PlayState* play) {
    ...
    if (BodyBreak_SpawnParts(&this->actor, &this->bodyBreak, play, this->actor.params + 4)) {
        ...
        if (ABS((s16)(this->actor.yawTowardsPlayer - this->actor.shape.rot.y)) <= 0x4000) {
            ...
            EnIk_CheckHealth(this,play);
                //EnIk_SetupBlock(this);
            //EnIk_SetupAttackJudge(this);
            //EnIk_BlockJudge(this, play);
        } else if( this->actor.xzDistToPlayer < 120 && this->actor.xzDistToPlayer >60 ) {
            EnIk_SetupInsCricleCut(this);
        }
        else{
            EnIk_SetupInsCricleCut(this);
...
void EnIk_SetupDead(EnIk* this) {
    ...
    EnIk_SetupAction(this, EnIk_Dead);
}
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if ((this->actor.colChkInfo.health == 0) && (this->unk_2F9 != 0)) {
            ...
            if (this->unk_2F9 == 0) {
                ...
                if (!CustomFlag_Get(0x5)) {
                    CustomFlag_Set(0x5,1);
                }
                
                Flags_SetSwitch(play, this->switchFlags);
                // Actor_Spawn(&play->actorCtx, play, ACTOR_ITEM_B_HEART, 1373, -1008, -429, 0, 0, 0, 0);             
                Actor_Spawn(&play->actorCtx, play, ACTOR_DOOR_WARP1,  this->actor.home.pos.x,  this->actor.home.pos.y,
                     this->actor.home.pos.z, 0, 0, 0, 0x14);
...
void EnIk_CheckHealth(EnIk* this, PlayState* play) {
    ...
    if ((this->unk_2FD == 0) || (this->unk_2FD == 0xD) || ((this->armorDown == 0) && (this->unk_2FD == 0xE))) {
        if (this->unk_2FD != 0) {
    ...
    if (this->actor.params != 0) {
        if ((prevHealth > 10) && (this->actor.colChkInfo.health < 50)) {//original were 10      
    ...
    } else if (this->actor.colChkInfo.health <= 50) {
    ...
    if ((this->actor.params != 0) && (this->armorDown == true)) {
        if ((prevHealth > 10) && (this->actor.colChkInfo.health <= 50)) {
            Audio_PlayActorSfx2(&this->actor, NA_SE_EN_IRONNACK_ARMOR_OFF_DEMO);
            EnIk_SetupArmorFall(this, play);
        } else {
            Audio_PlayActorSfx2(&this->actor, NA_SE_EN_IRONNACK_DAMAGE);
            Audio_PlayActorSfx2(&this->actor, NA_SE_EN_NUTS_CUTBODY);
            EnIk_SetupNoArmorTakeDamage(this);
        }
...
void func_80A75FA0(Actor* thisx, PlayState* play) {
    ...
    // Collider_UpdateCylinder(&this->actor, &this->lightningCollider);
...
void EnIk_CutChoose(EnIk* this) {
    ...
    {
        this->cutAction = VERTICALCUT;

    }
    else {
        this->cutAction = CROSSCUT;
    }
...
void EnIk_PostLimbDraw3(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 12) {
        ...
        Matrix_MultVec3f(&D_80A7847C[0], &this->axePos);//CUSTOM AXEPOS
        
        //Matrix_MultVec3f(&D_80A7847C[0], &this->axeForLightning->world.pos);
