...
#define SPAWN_PUNISH 0x30

#define MERGE_MASTER 0x40
#define MERGE_SLAVE 0x20

#define SMALLSIZE 0.004f
...
void EnFloormas_SetupJumpBack(EnFloormas* this);
void EnFloormas_JumpBack(EnFloormas* this, PlayState* play);
...
void EnFloormas_Init(Actor* thisx, PlayState* play2) {
    ...
    if (this->actor.params == SPAWN_PUNISH) {
        Actor_SetScale(&this->actor, SMALLSIZE);
        this->actor.colChkInfo.health = 1;
   }
    //Actor_SetScale(&this->actor, 0.004f); //CUSTOM SCALE
    // s16 cast needed
...
void EnFloormas_SetupTurn(EnFloormas* this) {
    ...
    if (this->actor.scale.x > SMALLSIZE) {
...
void EnFloormas_SetupLand(EnFloormas* this) {
    ...
    if ((this->actor.speedXZ < 0.0f) || (this->actionFunc != EnFloormas_Charge)) {
        if (this->actor.scale.x > SMALLSIZE) {
            this->actionTimer = 1;//original was 30
        }
        else {
            this->actionTimer = 15;//original was 45
        }
    }
    else
    {
...
void EnFloormas_SetupSplit(EnFloormas* this) {
    ...
    Actor_SetScale(&this->actor, SMALLSIZE); //original was 0.004f
...
void EnFloormas_SetupTakeDamage(EnFloormas* this) {
    ...
    Animation_Change(&this->skelAnime, &gWallmasterDamageAnim, 1.5f, 0, Animation_GetLastFrame(&gWallmasterDamageAnim), ANIMMODE_ONCE, -3.0f);
...
void EnFloormas_SetupRecover(EnFloormas* this) {
    ...
    Animation_Change(&this->skelAnime, &gWallmasterRecoverFromDamageAnim, 2.5f, 0, Animation_GetLastFrame(&gWallmasterRecoverFromDamageAnim), ANIMMODE_ONCE, 0.0f);
...
void EnFloormas_Die(EnFloormas* this, PlayState* play) {
    if (this->actor.scale.x > SMALLSIZE) { //the original was 0.004f
        // split
...
s8 EnFloormas_CanGrab(EnFloormas* this, PlayState* play) {
        Player* player = GET_PLAYER(play);

    if(!(player->stateFlags1 & (PLAYER_STATE1_IN_DEATH_CUTSCENE | PLAYER_STATE1_HANGING_FROM_LEDGE_SLIP   |
                                     PLAYER_STATE1_FREEFALLING )) &&
            !(player->stateFlags2 & PLAYER_STATE2_RESTRAINED_BY_ENEMY) && (player->meleeWeaponState != 1)){
                return 1;
            }else{
                return 0;
            }
...
void EnFloormas_BigDecideAction(EnFloormas* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->actor.xzDistToPlayer < 600.0f && !Actor_IsFacingPlayer(&this->actor, 0x4000)) {
            this->actionTarget = this->actor.yawTowardsPlayer;
            if(this->actor.xzDistToPlayer < 300.0f){
                EnFloormas_SetupJumpBack(this);
            }else{
                EnFloormas_SetupTurn(this);
            }
            // within 280 units of link and within 45 degrees rotation of him
        }
        else if ((this->actor.xzDistToPlayer < 380.0f) && Actor_IsFacingPlayer(&this->actor, 0x2000)&&(this->actor.xzDistToPlayer >180.0f)) {
...
void EnFloormas_Stand(EnFloormas* this, PlayState* play) {
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        } else if (this->actor.params == MERGE_SLAVE || this->actor.params == SPAWN_PUNISH) {
...
void EnFloormas_BigWalk(EnFloormas* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer < 380.0f) && (Actor_IsFacingPlayer(&this->actor, 0x4000)) ) {
    ...
    } else if ((this->actor.xzDistToPlayer < 600.0f) && !Actor_IsFacingPlayer(&this->actor, 0x4000)) {
...
void EnFloormas_Run(EnFloormas* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer < 380.0f) && Actor_IsFacingPlayer(&this->actor, 0x2000) &&
        !(this->actor.bgCheckFlags & BGCHECKFLAG_WALL)&& (this->actor.xzDistToPlayer > 180.0f)) {
        EnFloormas_SetupHover(this, play);
    } else if (this->actor.xzDistToPlayer >= 380.0f) {
        EnFloormas_SetupBigWalk(this);
    }else if ((this->actor.xzDistToPlayer <= 180.0f) && (Actor_IsFacingPlayer(&this->actor, 0x2000))) {       
          if(EnFloormas_CanGrab(this,play) == 1 && player->meleeWeaponState != 0){
...
void EnFloormas_Land(EnFloormas* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND_TOUCH) {
        if (this->actor.params != MERGE_MASTER || (this->actor.scale.x > SMALLSIZE) ) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->actionTimer == 0 && isOnGround) {
            ...
            } else if ((this->actor.params == MERGE_MASTER) && (this->actor.scale.x <= SMALLSIZE) ) {
                EnFloormas_SetupMerge(this);
            } else {                                    
                    EnFloormas_SetupStand(this);
                    this->smActionTimer = 50;// original was 50
                }
        }
...
void EnFloormas_SmWalk(EnFloormas* this, PlayState* play) {
    ...
    if (this->smActionTimer == 0) {
        ...
        this->actionTarget = this->actor.wallYaw;
        EnFloormas_SetupTurn(this);
    } else if (this->actor.xzDistToPlayer < 160.0f) { //original was 120.0f
...
void EnFloormas_SmDecideAction(EnFloormas* this, PlayState* play) {
    ...
    if (this->actor.params == MERGE_SLAVE) {
        ...
        if (Actor_WorldDistXZToActor(&this->actor, primaryFloormas) < 80.0f) {//original was 80.0f
            EnFloormas_SetupSlaveJumpAtMaster(this);
        }
    } else {
        Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 3, 0x71C);
             if(this->actor.xzDistToPlayer < 150.0f ){//original was 80.0f
            EnFloormas_SetupJumpAtLink(this);
        }
    }
}
...
void EnFloormas_SetupJumpBack(EnFloormas* this){
    Animation_Change(&this->skelAnime, &gWallmasterJumpAnim, 2.0f, 0.0f, 41.0f, ANIMMODE_ONCE, 0.0f);
    this->actionFunc = EnFloormas_JumpBack;
    this->actor.speedXZ = 0.0f;
}
...
void EnFloormas_JumpBack(EnFloormas* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame >= 20.0f) {
        Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 2, 0x438);
    }
    if (Animation_OnFrame(&this->skelAnime, 20.0f)) {   
        if (this->actor.scale.x > SMALLSIZE) {
            if (this->actor.xzDistToPlayer <= 80) {
                this->actor.speedXZ = -10.0f;
                this->actor.velocity.y = 8.6f;
            }
            else {
                this->actor.speedXZ = -9.0f; //Original was 5.0f
                this->actor.velocity.y = 8.5f;  //Original was 7.0f
            }
        }
        else {
        ...
        this->actionTimer = 0x32; //Original was 0x32
        this->actor.speedXZ = 0.0f;//Original was 0.0f
        Audio_PlayActorSfx2(&this->actor, NA_SE_EN_FLOORMASTER_SM_LAND);

        if ((Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360))&& (this->actor.xzDistToPlayer <= 230.0f) && (EnFloormas_CanGrab(this,play) == 1)) {
                EnFloormas_SetupJumpAtLink(this);
...
void EnFloormas_JumpAtLink(EnFloormas* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame < 20.0f) {
        ...
        if (this->actor.scale.x > SMALLSIZE) {//Bigfloormaster
            this->actor.speedXZ = 16.0f; //Original was 5.0f
            this->actor.velocity.y = 7.0f;  //Original was 7.0f
        }
        else if(this->actor.xzDistToPlayer> 110) {
            this->actor.speedXZ = 18.0f;
            this->actor.velocity.y = 6.1f;
        }
        else if(this->actor.xzDistToPlayer<=110){
            this->actor.speedXZ = 10.0f;
...
void EnFloormas_GrabLink(EnFloormas* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        if ((this->suckCounter > 0) && (this->actor.scale.x > SMALLSIZE)) {
            if (this->tempTimer <= 2.0f) {
            ...
            if (this->skelAnime.playSpeed > 0.0f) {
                this->skelAnime.playSpeed = -1.0f -ABS(this->tempTimer);//original was -1.0f
            ...
            if (this->skelAnime.playSpeed > 0.0f) {
                this->skelAnime.playSpeed = -1.0f;//original was -1.0f
                this->skelAnime.endFrame = 36.0f;
                this->skelAnime.startFrame = 45.0f;
            }
            else {
    ...
    // let go
    if (!(player->stateFlags2 & PLAYER_STATE2_RESTRAINED_BY_ENEMY) || (player->invincibilityTimer < 0) || (player->actor.parent == NULL)) {
        ...
        if ((this->suckCounter >= 4) &&(this->actor.scale.x > SMALLSIZE)) {
            
            func_8002F7DC(&player->actor, NA_SE_EN_GOMA_BJR_EGG1);
            func_8002F7DC(&player->actor, NA_SE_EN_GOMA_CRY2);
            Actor_Spawn(&play->actorCtx, play, ACTOR_EN_FLOORMAS, this->tailPos.x, this->tailPos.y,
                this->tailPos.z, 0, 0, 0, SPAWN_PUNISH);
            this->spawnCounter++;
            this->suckCounter = 0;
            this->tempTimer = 0;
        }
        // Damage link every 20 frames
         if ((this->actionTarget % 20) == 0) {
             ...
             if (this->actor.scale.x <= SMALLSIZE) {
                 ...
                 if ((this->actor.colChkInfo.health >= sColChkInfoInit.health) &&(this->spawnCounter < 2)) {
                     this->suckCounter++;
                 }
                 else if(this->actor.colChkInfo.health < sColChkInfoInit.health) {
...
void EnFloormas_Merge(EnFloormas* this, PlayState* play) { //also control master's recorvey
    EnFloormas* parent;
    EnFloormas* child;
    s32 mergeCnt;
    f32 prevScale;
    ...
    if (((prevScale == 0.007f) || (prevScale == SMALLSIZE)) && (prevScale != this->actor.scale.x)) {
...
void EnFloormas_ColliderCheck(EnFloormas* this, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        Actor_SetDropFlag(&this->actor, &this->collider.info, true);
         if ((this->actor.scale.x <= SMALLSIZE) && this->collider.info.acHitInfo->toucher.dmgFlags & DMG_HOOKSHOT) {
                    this->actor.colChkInfo.damage = 2;
                    this->actor.colChkInfo.damageEffect = 0;
                }
                if ((this->actor.scale.x <= SMALLSIZE) && this->collider.info.acHitInfo->toucher.dmgFlags & DMG_ARROW_NORMAL) {
                    this->actor.colChkInfo.damage = 2;
                    this->actor.colChkInfo.damageEffect = 0;
                }
...
void EnFloormas_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnFloormas_SmWait) {
        ...
        if (this->actionFunc != EnFloormas_GrabLink) {
            ...
            if ((this->actionFunc != EnFloormas_SmSlaveJumpAtMaster) || (this->skelAnime.curFrame < 20.0f)|| (this->actionFunc != EnFloormas_GrabLink)) {
...
void EnFloormas_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx) {
    Vec3f TailOffset = { 0.0f, 4000.0f, 0.0f };
    EnFloormas* this = (EnFloormas*)thisx;
    ...
    if (limbIndex == 17) {
        Matrix_MultVec3f(&TailOffset, &this->tailPos);
    }
}
