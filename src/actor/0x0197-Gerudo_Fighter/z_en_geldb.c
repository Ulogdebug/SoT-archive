...
static DamageTable sDamageTable = {
    ...
    /* Slingshot     */ DMG_ENTRY(0, GELDB_DMG_UNK_6),
    /* Explosive     */ DMG_ENTRY(1, GELDB_DMG_NORMAL),
    /* Boomerang     */ DMG_ENTRY(0, GELDB_DMG_UNK_6),
    /* Normal arrow  */ DMG_ENTRY(2, GELDB_DMG_NORMAL),
    /* Hammer swing  */ DMG_ENTRY(4, GELDB_DMG_NORMAL),
    /* Hookshot      */ DMG_ENTRY(0, GELDB_DMG_UNK_6),
    ...
    /* Hammer jump   */ DMG_ENTRY(8, GELDB_DMG_NORMAL),
...
void EnGeldB_Init(Actor* thisx, PlayState* play) {
    ...
    this->keyFlag = (this->actor.params >> 8) & 0xFF;
    thisx->params &= 0xFF;
    this->blinkState = 0;
    this->unkFloat = 10.0f;
    ...
    Actor_SetScale(thisx, 0.013f);//012499999f, wtf?
    EnGeldB_SetupWait(this);
    if ((this->keyFlag != 0) && Flags_GetSwitch(play, this->keyFlag)) {
        Actor_Kill(thisx);
    }
}
...
void EnGeldB_RollForward(EnGeldB* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        } else if ((Rand_ZeroOne() > 0.5f || (ABS(facingAngleToLink) < 0x3FFC))) {
            EnGeldB_SetupSlash(this);
        } else {
            EnGeldB_SetupAdvance(this, play);
        }
...
void EnGeldB_Defeated(EnGeldB* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (!Actor_FindNearby(play, &this->actor, ACTOR_EN_GELDB, ACTORCAT_ENEMY, 8000.0f)) {
            func_800F5B58();
            if(this->keyFlag != 0){
                Flags_SetSwitch(play, this->keyFlag);
            }
        }
    }
}
...
s32 EnGeldB_DodgeRanged(PlayState* play, EnGeldB* this) {
    ...
    if (actor != NULL) {
        ...
        if ((ABS(angleToFacing) < 0x2EE0) && (dist < 600.0f)) {
