...
void func_80890740(BgIceShelter* this, PlayState* play) {
    ...
    this->cylinder1.dim.radius = cylinderRadii[type] + 5;
    this->cylinder1.dim.height = cylinderHeights[type] + 5;
...
void BgIceShelter_Init(Actor* thisx, PlayState* play) {
    ...
    if((play->sceneId != 0x7) && (play->sceneId != 0x3C)){
        if (/*!((this->dyna.actor.params >> 6) & 1) &&*/ Flags_GetSwitch(play, this->dyna.actor.params & 0x3F)) {
            Flame = Actor_Spawn(&play->actorCtx, play, ACTOR_EN_ICE_HONO, this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y, this->dyna.actor.world.pos.z, 0, 0, 0, 0xFFFF);
...
void func_808911D4(BgIceShelter* this, PlayState* play) {
    ...
    else if(this->alpha == 55){
        if(play->sceneId != 0x7){
            OnePointCutscene_AttentionSetSfx(play, &this->dyna.actor, NA_SE_SY_CORRECT_CHIME);
            if (/*!((this->dyna.actor.params >> 6) & 1) &&*/ ((play->sceneId != 0x7) && (play->sceneId != 0x3C))) {       
            Flame = Actor_Spawn(&play->actorCtx, play, ACTOR_EN_ICE_HONO, this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y, this->dyna.actor.world.pos.z, 0, 0, 0, 0xFFFF);
            }
        }
    }
