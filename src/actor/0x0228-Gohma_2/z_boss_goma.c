...
static InitChainEntry sInitChain[] = {
    ...
    ICHAIN_S8(naviEnemyId, 0xC2, ICHAIN_CONTINUE),
...
void BossGoma_Init(Actor* thisx, PlayState* play) {
    ...
    this->actor.world.pos.y = this->actor.home.pos.y; // ceiling
...
void BossGoma_UpdateCeilingMovement(BossGoma* this, PlayState* play, f32 dz, f32 targetSpeedXZ,
    ...
    static Vec3f roomCenter = { 0.0f, 0.0f, 0.0f };
    Vec3f* basePos = NULL;
    s16 i;
    Vec3f vel;
    Vec3f accel;
    ...
    if (rotateTowardsCenter) {
        Math_ApproachS(&this->actor.world.rot.y, Math_Vec3f_Yaw(&this->actor.world.pos, &this->actor.home.pos) + 0x8000, 3,
                       0x3E8);
    }
...
void BossGoma_SetupEncounterState4(BossGoma* this, PlayState* play) {
    ...
    //player->actor.world.pos.x = player->actor.home.pos.x;
    //player->actor.world.pos.z = player->actor.home.pos.z;
...
void BossGoma_Encounter(BossGoma* this, PlayState* play) {
    ...
    switch (this->actionState) {
        case 0: // wait for the player to enter the room
            ...
            if (fabsf(player->actor.world.pos.x - player->actor.world.pos.x) < 60.0f &&
                fabsf(player->actor.world.pos.z - player->actor.world.pos.z) < 60.0f) {
        ...
        case 1: // player entered the room
            ...
            //this->subCamEye.y = player->actor.world.pos.y;
...
void BossGoma_CeilingMoveToCenter(BossGoma* this, PlayState* play) {
    ...
    if (this->framesUntilNextAction == 0 && fabsf(this->actor.home.pos.x - this->actor.world.pos.x) < 100.0f &&
        fabsf(this->actor.home.pos.z - this->actor.world.pos.z) < 100.0f) {
        BossGoma_SetupCeilingIdle(this);
    }
}
        ...
        if (this->eyeClosedTimer == 0 && this->actionFunc != BossGoma_CeilingSpawnGohmas &&
            ...
            if (this->actionFunc == BossGoma_CeilingMoveToCenter || this->actionFunc == BossGoma_CeilingIdle ||
                ...
                this->invincibilityFrames = 10;
                       ...
                       (acHitInfo->toucher.dmgFlags & (DMG_SLINGSHOT | DMG_DEKU_NUT | DMG_ARROW | DMG_HOOKSHOT))) {
                Audio_PlayActorSfx2(&this->actor, NA_SE_EN_GOMA_DAM2);
...
void BossGoma_Update(Actor* thisx, PlayState* play) {
    ...
    if (!this->disableGameplayLogic) {
        ...
        if (this->actionFunc != BossGoma_FloorStunned && this->actionFunc != BossGoma_FloorDamaged && this->actionFunc != BossGoma_Defeated &&
