...
void EnDodongo_Walk(EnDodongo* this, PlayState* play) {
    ...
    Math_SmoothStepToF(&this->actor.speedXZ, 2.5f, 0.1f, 1.0f, 0.0f);//the original end speed is 1.5f
    ...
    if (Math_Vec3f_DistXZ(&this->actor.world.pos, &player->actor.world.pos) < 400.0f) {
        ...
        this->actor.flags &= ~ACTOR_FLAG_0;
        if ((Math_Vec3f_DistXZ(&this->actor.world.pos, &this->actor.home.pos) > 600.0f) || (this->retreatTimer != 0)) {
...
void EnDodongo_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND_TOUCH) {
        ...
        if ((floorProperty == FLOOR_PROPERTY_5) || (floorProperty == FLOOR_PROPERTY_12) || (floorProperty == FLOOR_TYPE_9)) {
            Actor_Kill(&this->actor);
            return;
        }
    }
