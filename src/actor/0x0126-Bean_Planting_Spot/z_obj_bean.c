...
void ObjBean_Init(Actor* thisx, PlayState* play) {
    s32 path;
    ObjBean* this = (ObjBean*)thisx;
    s16 switchFlag = this->dyna.actor.params & 0x3F;
    Actor_ProcessInitChain(&this->dyna.actor, sInitChain);
    if (LINK_AGE_IN_YEARS == YEARS_ADULT) {
        if (Flags_GetSwitch(play, switchFlag) || (mREG(1) == 1) || switchFlag == 0x3F) {
    ...
    } else if ((Flags_GetSwitch(play, switchFlag) != 0) || (mREG(1) == 1)) {
...
void ObjBean_Fly(ObjBean* this, PlayState* play) {
    ...
    if (this->currentPointIndex == this->pathCount) {
        ...
        if ((mainCam->setting == CAM_SET_BEAN_LOST_WOODS) || (mainCam->setting == 0x3A)) {
