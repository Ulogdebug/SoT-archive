...
void EnJso_FallDown(EnJso* this, PlayState* play);
void EnJso_TellHint(EnJso* this, PlayState* play);
void EnJso_BurstIntoFlames(EnJso* this, PlayState* play);
void EnJso_BurstIntoFlames2(EnJso* this, PlayState* play);
void EnJso_Appear(EnJso* this, PlayState* play);
void EnJso_SetupWaitGround(EnJso* this,PlayState* play);
void EnJso_WaitGround(EnJso* this,PlayState* play);
...
static DamageTable sDamageTable = {
    ...
    /* Boomerang      */ DMG_ENTRY(0, EN_JSO_DMGEFF_STUN),
    /* Normal arrow   */ DMG_ENTRY(1, EN_JSO_DMGEFF_NONE),
    /* Hammer swing   */ DMG_ENTRY(4, EN_JSO_DMGEFF_NONE),
    ...
    /* Fire magic     */ DMG_ENTRY(2, EN_JSO_DMGEFF_FIRE),
    /* Ice magic      */ DMG_ENTRY(2, EN_JSO_DMGEFF_FREEZE),
    /* Light magic    */ DMG_ENTRY(2, EN_JSO_DMGEFF_LIGHT_ORB),
    /* Normal shield  */ DMG_ENTRY(0, EN_JSO_DMGEFF_IMMUNE),
    /* Light ray      */ DMG_ENTRY(0, EN_JSO_DMGEFF_IMMUNE),
    /* Kokiri spin    */ DMG_ENTRY(2, EN_JSO_DMGEFF_NONE),
    /* Giant spin     */ DMG_ENTRY(8, EN_JSO_DMGEFF_NONE),
    /* Master spin    */ DMG_ENTRY(4, EN_JSO_DMGEFF_NONE),
    /* Kokiri jump    */ DMG_ENTRY(1, EN_JSO_DMGEFF_NONE),
    /* Giant jump     */ DMG_ENTRY(8, EN_JSO_DMGEFF_NONE),
    /* Master jump    */ DMG_ENTRY(4, EN_JSO_DMGEFF_NONE),
    /* Unknown 1      */ DMG_ENTRY(0, EN_JSO_DMGEFF_IMMUNE),
    /* Unblockable    */ DMG_ENTRY(0, EN_JSO_DMGEFF_IMMUNE),
    /* Hammer jump    */ DMG_ENTRY(8, EN_JSO_DMGEFF_NONE),
    /* Unknown 2      */ DMG_ENTRY(0, EN_JSO_DMGEFF_IMMUNE),
};
...
static AnimationHeader* sAnimations[EN_JSO_ANIM_MAX] = {
    0x060081f4,      // EN_JSO_ANIM_APPEAR
    0x0600ae80,        // EN_JSO_ANIM_IDLE
    0x0600b5f0,      // EN_JSO_ANIM_BOUNCE
    0x06004858,       // EN_JSO_ANIM_GUARD
    0x06003530,  // EN_JSO_ANIM_DASH_ATTACK
    0x0600378c,  // EN_JSO_ANIM_SLASH_START
    0x060038ac,   // EN_JSO_ANIM_SLASH_LOOP
    0x0600603c, // EN_JSO_ANIM_KNOCKED_BACK
    0x06007b04,       // EN_JSO_ANIM_COWER
    0x060044f8,     // EN_JSO_ANIM_DAMAGED
    0x06003d5c,    // EN_JSO_ANIM_JUMP_BACK
    0x06005d5c,    // EN_JSO_ANIM_FALL_DOWN
};
...
void EnJso_Init(Actor* thisx, PlayState* play) {
    ...
    this->actor.naviEnemyId = NAVI_ENEMY_VOLVAGIA;
    ...
    SkelAnime_InitFlex(play, &this->skelAnime, 0x0600aa00, 0x0600ae80, this->jointTable, this->morphTable,
                       GARO_LIMB_MAX);
    ...
    EnJso_SetupWaitGround(this,play);
    
}

void EnJso_SetupWaitGround(EnJso* this,PlayState* play){
    this->scale = 0.0f;
    this->actor.world.pos.y = this->actor.home.pos.y - 20.5f;
    this->actor.flags &= ~ACTOR_FLAG_0;
    this->actionFunc = EnJso_WaitGround;
}

void EnJso_WaitGround(EnJso* this,PlayState* play){
    if ((ABS(this->actor.yDistToPlayer) < 60.0f) && this->actor.xzDistToPlayer < 400.0f) {
        // this->actor.world.rot.y = this->actor.yawTowardsPlayer;
        // this->actor.shape.rot.y = this->actor.yawTowardsPlayer;

        EnJso_SetupAppear(this,play);
    }
}
...
void EnJso_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (!Actor_FindNearby(play, &this->actor, 0x025A, ACTORCAT_ENEMY, 8000.0f)) {
        func_800F5B58();
    }
...
void EnJso_HandleIntroCutscene(EnJso* this, PlayState* play) {
    ...
    switch (this->introCsState) {
        case EN_JSO_INTRO_CS_STATE_DONE_OR_STARTED:
            ...
            this->actor.draw = EnJso_Draw;
            //Audio_PlayActorSfx2(&this->actor, NA_SE_EN_ANSATSUSYA_ENTRY);
            SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_APPEAR, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);            
            this->actor.shape.yOffset = 970.0f;
        ...
        case EN_JSO_INTRO_CS_STATE_RAISE_ARM_AND_DRAW_RIGHT_SWORD:
            ...
            if (this->targetRobeRightRot.x != 0) {
                if ((ABS_ALT(diffToTargetRobeRightRot) < 0x64) && (this->swordState == EN_JSO_SWORD_STATE_NONE_DRAWN)) {
                    ...
                    SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_SWORD, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
                }
            }
...
void EnJso_SetupReappear(EnJso* this, PlayState* play) {
    ...
    this->actor.flags |= ACTOR_FLAG_27;
    //Audio_PlayActorSfx2(&this->actor, NA_SE_EN_ANSATSUSYA_ENTRY);
    SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_APPEAR, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);    this->afterimageCount = 0;
...
void EnJso_SetupAppear(EnJso* this, PlayState* play) {
    EnEncount3* parent = (EnEncount3*)this->actor.parent;
    this->actor.speedXZ = 0.0f;
    this->actor.velocity.y = 0.0f;
    EnJso_ChangeAnim(this, EN_JSO_ANIM_APPEAR);
    ...
    this->action = EN_JSO_ACTION_REAPPEAR;
    this->actor.gravity = 0.0f;
    Math_ApproachF(&this->scale, 0.035f, 0.5f, 0.08f);
    if (this->scale >= 0.034f) {
        this->scale = 0.035f;
        this->actor.world.pos.y = this->actor.home.pos.y;
        SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_APPEAR, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnJso_CirclePlayer(EnJso* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 12.0f)) {
        SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_SKIP, 0.1f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
        this->actor.speedXZ = 0.0f;
...
void EnJso_SetupDashAttack(EnJso* this) {
    ...
    //Audio_PlayActorSfx2(&this->actor, NA_SE_EN_ANSATSUSYA_ENTRY);
    SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_APPEAR, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    //Audio_PlayActorSfx2(&this->actor, NA_SE_EN_ANSATSUSYA_DASH_2);
    SoundEffect_PlayHeld(SOUND_ENEMY_GARO_DASH, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
    ...
    if ((this->rightSwordCollider.base.atFlags & AT_BOUNCED) || (this->leftSwordCollider.base.atFlags & AT_BOUNCED)) {
        ...
        if (this->swordState == EN_JSO_SWORD_STATE_BOTH_DRAWN) {
            ...
            //AudioSfx_SetChannelIO(&this->actor.projectedPos, NA_SE_EN_ANSATSUSYA_DASH_2, 0);
              func_8002F71C(play, &this->actor, CLAMP(this->actor.speedXZ - 2.0f,4.0f,8.0f),
            this->actor.world.rot.y, CLAMP(this->actor.speedXZ - 2.0f,6.0f,8.0f));
            SoundEffect_PlayHeld(SOUND_ENEMY_GARO_DASH, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnJso_SetupWaitAfterSlash(EnJso* this) {
    ...
    if (this->swordHitSomething) {
        ...
        EnJso_ChangeAnim(this, EN_JSO_ANIM_SLASH_LOOP);
        if((this->actor.xzDistToPlayer < 200.0f) && (absYawDiff < 0x4300)){
            this->timer = 0; 
        }else{
            this->timer = 10;
        }
...
void EnJso_SetupDamaged(EnJso* this, PlayState* play) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_DAMAGE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnJso_SetupDead(EnJso* this, PlayState* play) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_DEAD, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 1000.0f, &this->actor);
...
void EnJso_FallDownAndTalk(EnJso* this, PlayState* play) {
    ...
    if (!Play_InCsMode(play) && (play->msgCtx.msgLength == 0)) {
        if (fabsf(player->actor.world.pos.y - this->actor.world.pos.y) < 60.0f) {
            ...
            player2->stateFlags1 |= PLAYER_STATE1_28;
...
void EnJso_SetupFallDown(EnJso* this, PlayState* play) {
    EnJso_ChangeAnim(this, EN_JSO_ANIM_FALL_DOWN);
    //this->textIndex = 2;
    this->actor.colChkInfo.mass = MASS_IMMOVABLE;
    this->actor.flags |= (ACTOR_FLAG_0 | ACTOR_FLAG_3);
    ...
    this->actor.flags &= ~ACTOR_FLAG_20;
    this->action = EN_JSO_ACTION_FALL_DOWN_AND_TALK;
    this->actionFunc = EnJso_FallDown;
}
...
void EnJso_FallDown(EnJso* this, PlayState* play) {
    ...
    this->actionFunc = EnJso_BurstIntoFlames2;
}
...
void EnJso_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == GARO_LIMB_LEFT_SWORD) {
        if (this->swordState == EN_JSO_SWORD_STATE_KNOCKED_OUT_OF_HANDS) {
            ...
            BodyBreak_SetInfo(&this->bodyBreak, limbIndex, 0, 15, 15, dList, BODYBREAK_OBJECT_DEFAULT);
        }
