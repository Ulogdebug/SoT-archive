...
void ObjBombiwa_Init(Actor* thisx, PlayState* play) {
    ...
    ObjBombiwa* this = (ObjBombiwa*)thisx;
    this->switchFlags = thisx->params & 0x3F;

    if ((Flags_GetSwitch(play, this->switchFlags) !=0 ) && (this->switchFlags != 0x00) && (this->switchFlags != 0x3F)) {
        Actor_Kill(thisx);
    } else {
...
void ObjBombiwa_Update(Actor* thisx, PlayState* play) {
    ...
    if ((func_80033684(play, &this->actor) != NULL) ||
        ...
        if ((this->switchFlags != 0x3F) && (this->switchFlags != 0x00)) {
