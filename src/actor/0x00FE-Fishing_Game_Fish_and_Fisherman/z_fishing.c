...
void Fishing_HandleAquariumDialog(Fishing* this, PlayState* play) {
    ...
    if (this->unk_1D3 == 0 && (!Flags_GetSwitch(play, 0x37))) {
        if (this->unk_1D4 == 0) {
...
void Fishing_UpdateFish(Actor* thisx, PlayState* play2) {
    ...
    if(play->sceneId == 0x49 && Flags_GetSwitch(play,0x37)){
        //Debug_Text(255, 0, 0, 1, 1, "Kill");
    ...
    switch (this->unk_15C) {
        ...
        case 5:
            if ((Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(play)) {
                ...
                play->interfaceCtx.unk_260 = 1; //in Fishing game Flag
...
void Fishing_UpdateOwner(Actor* thisx, PlayState* play2) {
    ...
    if(Flags_GetSwitch(play, 0x37) && !this->startTextBoxFlag){
        Message_StartTextbox(play, 0x5017, NULL); //Say the game close
        this->startTextBoxFlag = 1;
    }
