...
#define SCENE_FOREST_TEMPLE_BOSS (0x1E)
#define SCENE_JABU_JABU_BOSS (0x3F)
...
void EnFhgFire_Init(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.params == FHGFIRE_LIGHTNING_SHOCK) || (this->actor.params == FHGFIRE_LIGHTNING_BURST) || (this->actor.params == FHGFIRE_LIGHTNING_BURST_RED) ||
    ...
    if (this->actor.params == FHGFIRE_LIGHTNING_SHOCK) {
        ...
        (this->actor.params == FHGFIRE_LIGHTNING_BURST_RED) {
        EnFhgFire_SetUpdate(this, EnFhgFire_LightningBurst);
        this->fwork[FHGFIRE_ALPHA] = 255.0f;
        this->work[FHGFIRE_TIMER] = 32;
        this->work[FHGFIRE_FX_TIMER] = 50;
        ...
        // Seems to start throwing this ball
        ...
        Lights_PointNoGlowSetInfo(&this->lightInfo, this->actor.world.pos.x, this->actor.world.pos.y,
            ...
            // this->ballcolor = (s16)Rand_ZeroFloat(1.99f);
...
void EnFhgFire_LightningStrike(EnFhgFire* this, PlayState* play) {
    ...
    switch (this->work[FHGFIRE_FIRE_MODE]) {
        ...
        case STRIKE_BURST:
            ...
            if (this->work[FHGFIRE_TIMER] == 0 || (this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_WALL | BGCHECKFLAG_CEILING))) {
                this->work[FHGFIRE_FIRE_MODE] = STRIKE_TRAILS;
                this->actor.shape.rot.z += 0x8000;
                //this->work[FHGFIRE_TIMER] = 37;
                this->work[FHGFIRE_TIMER] = 47;
        ...
        case STRIKE_TRAILS:
            ...
            if ((this->work[FHGFIRE_TIMER] == 30) && (play->sceneId == SCENE_JABU_JABU_BOSS))  {
                Actor_Kill(&this->actor);
            } else if (this->work[FHGFIRE_TIMER] == 0) {
                Actor_Kill(&this->actor);
            }
    }
...
void EnFhgFire_EnergyBall(EnFhgFire* this, PlayState* play) {
    ...
    if (this->work[FHGFIRE_KILL_TIMER] != 0) {
        ...
        // If commented, the ball doesn't appear and I am attacked directly
        ...
        dzL = player->actor.world.pos.z - this->actor.world.pos.z;
        // If these two func are commented, the ball appear but doesn't move
        ...
        this->actor.shape.rot.z += (s16)(Rand_ZeroOne() * 0x4E20) + 0x4000;
        {
            // Lightballcolor1 = particles behind the ball
           u8 lightBallColor1 = FHGFLASH_LIGHTBALL_GREEN;
            ...
            else if (this->work[FHGFIRE_FIRE_MODE] == FHGFIRE_LIGHT_RED) { 
                lightBallColor1 = FHGFLASH_LIGHTBALL_RED;
            }
            else { 
                lightBallColor1 = FHGFLASH_LIGHTBALL_GREEN;
            }
            for (i1 = 0; i1 < 3; i1++) {
        ...
        // This one still throw ball but stops hurting + impossible to return it.
        switch (this->work[FHGFIRE_FIRE_MODE]) {
            // If this case est commented, it will not hurt Link + impossible to counter it.
           case FHGFIRE_LIGHT_GREEN:
               ...
               canBottleReflect1 =
                ...
                if ((this->collider.base.acFlags & AC_HIT) || canBottleReflect1) {
                    ...
                    if (!canBottleReflect2 && (hurtbox->toucher.dmgFlags & DMG_SHIELD)) { //manage to defend by player's shield
                        ...
                        if (bossGnd->flyMode == GND_FLY_NEUTRAL) {
                            ...
                            if ((this->work[FHGFIRE_RETURN_COUNT] > 5) && (Rand_ZeroOne() < 0.5f)) {
                ...
                } else if (sqrtf(SQ(dxL) + SQ(dyL) + SQ(dzL)) <= 25.0f) { // too close and hit player
                    //this->actor.flags &= ~(ACTOR_FLAG_7);
                    killMode = BALL_BURST;
                    Audio_PlayActorSfx2(&this->actor, NA_SE_EN_FANTOM_HIT_THUNDER);
           ...
           case FHGFIRE_LIGHT_BLUE:
               if ((bossGnd->flyMode == GND_FLY_RETURN) && (this->work[FHGFIRE_RETURN_COUNT] < 100) || bossGnd->flyMode == GND_FLY_CHARGE) {
                    this->actor.world.rot.y = RAD_TO_BINANG(Math_FAtan2F(dxPG, dzPG)); //manage to hit back
                    ...
                    if ((fabsf(dxPG) < 30.0f) && (fabsf(dzPG) < 30.0f) && (fabsf(dyPG) < 45.0f)) {
                        //this->actor.flags &= ~(ACTOR_FLAG_7);
                        killMode = BALL_IMPACT; //Hit ganon
                ...
                if (this->work[FHGFIRE_TIMER] == 0) {
                    ...
                    for (i3 = 0; i3 < 30; i3++) {
                        ...
                        EffectSsFhgFlash_SpawnLightBall(play, &this->actor.world.pos, &sp88, &sp7C,
                                                        (s16)(Rand_ZeroOne() * 40.0f) + 80, FHGFLASH_LIGHTBALL_LIGHTBLUE);
                    }
                    this->actor.world.rot.y = RAD_TO_BINANG(Math_FAtan2F(dxL, dzL));
                    dxzL = sqrtf(SQ(dxL) + SQ(dzL));
                    this->actor.world.rot.x = RAD_TO_BINANG(Math_FAtan2F(dyL, dxzL));
        ...
        if (this->work[FHGFIRE_FX_TIMER] == 0) {
            Actor_UpdateBgCheckInfo(play, &this->actor, 50.0f, 50.0f, 100.0f,
                                    ...
                                    // This will spawn green particles when the ball is killed
            if ((this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_WALL | BGCHECKFLAG_CEILING)) ||
                killMode) {
                    // Lightballcolor2 = particles when the ball dies
                ...
                if (this->work[FHGFIRE_FIRE_MODE] == FHGFIRE_LIGHT_GREEN) {
                    lightBallColor2 = FHGFLASH_LIGHTBALL_GREEN;
                } else if (this->work[FHGFIRE_FIRE_MODE] == FHGFIRE_LIGHT_RED) {
                    lightBallColor2 = FHGFLASH_LIGHTBALL_RED;
                } else {
                    lightBallColor2 = FHGFLASH_LIGHTBALL_BLUE;
                }
                for (i4 = 0; i4 < 30; i4++) {
                ...
                else if (killMode == BALL_BURST_RED) {
                    Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_EN_FHG_FIRE, this->actor.world.pos.x,
                                       player->actor.world.pos.y + 20.0f, this->actor.world.pos.z, 0xC8, 0, 0,
                                       FHGFIRE_LIGHTNING_BURST_RED);
                }
...
void EnFhgFire_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params == FHGFIRE_LIGHTNING_BURST) {
        ...
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 255, 255, (s8)this->fwork[FHGFIRE_ALPHA]); //blue, not this
        ...
        else if (this->actor.params == FHGFIRE_LIGHTNING_BURST_RED) {
           Gfx_SetupDL_25Xlu(play->state.gfxCtx);
        ...
        if (this->work[FHGFIRE_FIRE_MODE] == FHGFIRE_LIGHT_GREEN) {
            // Blue ball from the player
            gDPSetEnvColor(POLY_XLU_DISP++, 165, 255, 75, 0);
        } else if (this->work[FHGFIRE_FIRE_MODE] == FHGFIRE_LIGHT_RED) {
            gDPSetEnvColor(POLY_XLU_DISP++, 255, 0, 0, 0);
        } else {
             //Green ball from the ennemy
