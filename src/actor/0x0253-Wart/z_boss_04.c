...
void Wart_Init(Actor* thisx, PlayState* play2);
void Wart_Destroy(Actor* thisx, PlayState* play);
void Wart_Update(Actor* thisx, PlayState* play2);
void Wart_Draw(Actor* thisx, PlayState* play);

void Wart_SetupPlayCutscene(Boss04* this);
void Wart_PlayCutscene(Boss04* this, PlayState* play);
void func_809ECD00(Boss04* this, PlayState* play);
void func_809ECD18(Boss04* this, PlayState* play);
void func_809ECEF4(Boss04* this);
void Wart_Move(Boss04* this, PlayState* play);
void Wart_SetupDie(Boss04* this);
void Wart_Die(Boss04* this, PlayState* play);
...
void Wart_UpdateColliderProperty(s32 index, ColliderJntSph* collider, Vec3f* arg2) {
...
void Wart_Init(Actor* thisx, PlayState* play2) {
    ...
    if (Flags_GetClear(play, play->roomCtx.curRoom.num)) {
        Actor_Spawn(&play->actorCtx, play, ACTOR_DOOR_WARP1,  this->actor.home.pos.x,  this->actor.home.pos.y,
                     this->actor.home.pos.z, 0, 0, 0, 0x15);
        //Actor_Spawn(&play->actorCtx, play, ACTOR_ITEM_B_HEART, 88, -14, 331, 0, 0, 0, 0);             
    ...
    Collider_SetJntSph(play, &this->collider1, &this->actor, &sJntSphInit1, this->collider1Elements);
    Collider_SetJntSph(play, &this->collider2, &this->actor, &sJntSphInit2, this->collider2Elements);
    SkelAnime_InitFlex(play, &this->skelAnime, 0x060045e8, 0x0600004c, this->jointTable, this->morphTable,
                       WART_LIMB_MAX);
    ...
    Actor_UpdateBgCheckInfo(play, &this->actor, 35.0f, 60.0f, 60.0f, UPDBGCHECKINFO_FLAG_2);
    ...
    for (i = 0; i < 82; i++) {
        ...
        Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, 0x0254, this->unk_6E8 + sp90.x, phi_f24,
                           this->unk_6F0 + sp90.z, 0, 0, 0, i);
        phi_f20 += 2.5f;
        phi_s0_2 += 0x1300;
    }

    Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, 0x0254, this->actor.world.pos.x,
...
void Wart_SetupPlayCutscene(Boss04* this) {
    this->actionFunc = Wart_PlayCutscene;
...
void Wart_PlayCutscene(Boss04* this, PlayState* play) {
    ...
    switch (this->unk_708) {
        case 0:
            ...
            if ((player->stateFlags1 & PLAYER_STATE1_20) && (this->actor.projectedPos.z > 0.0f) &&
                (fabsf(this->actor.projectedPos.x) < 300.0f) && (fabsf(this->actor.projectedPos.y) < 300.0f)) {
                if ((this->unk_704 >= 15) && (CutsceneManager_GetCurrentCsId() == -1)) {
                    Actor* boss;
                    ...
                    Play_ChangeCameraStatus(play, CAM_ID_MAIN, 1);
                    Play_ChangeCameraStatus(play, this->subCamId, 7);
                    ...
                    boss = play->actorCtx.actorLists[ACTORCAT_BOSS].head;
                    while (boss != NULL) {
                        if (boss->id == 0x0201) {
                            Actor_Kill(boss);
                        }
        ...
        case 12:
            ...
            SoundEffect_PlayHeld(SOUND_ENEMY_WART_ATTACK, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor);      
            ...
            if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND_TOUCH) {
                ...
                SoundEffect_PlayOneshot(SOUND_POWDER_KEG_EXPLODE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor);
                this->unk_6F4 = 15;
                this->unk_708 = 13;
                this->unk_704 = 0;
                this->unk_2DA = 10;
                Actor_Spawn(&play->actorCtx, play, 0x033B, this->actor.world.pos.x, this->actor.world.pos.y,
                            this->actor.world.pos.z, 0, 0, 0, 0x23);
                //Actor_PlaySfx(&this->actor, NA_SE_EN_KONB_JUMP_LEV_OLD - SFX_FLAG);
                SoundEffect_PlayHeld(SOUND_BOSS_GYORG_JUMP_LOOP, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor); 
    ...
    this->actor.shape.rot.x = Math_Atan2S(sqrtf(SQ(x) + SQ(z)), -y);
}
...
void func_809ECD18(Boss04* this, PlayState* play) {
    ...
    Math_ApproachS(&this->actor.world.rot.y,
                   Math_Atan2S(this->unk_6C8.z - this->actor.world.pos.z, this->unk_6C8.x - this->actor.world.pos.x), 5,
                   0x200);
...
void Wart_SetupDie(Boss04* this) {
    ...
    SoundEffect_PlayOneshot(SOUND_ENEMY_WART_DEAD, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor);
    this->actor.flags &= ~ACTOR_FLAG_0;
    func_800F5B58();//Disable BGM;
...
void Wart_Die(Boss04* this, PlayState* play) {
    ...
    if ((this->unk_1F8 == 2) || (this->unk_1F8 == 5)) {
        Actor_Spawn(&play->actorCtx, play, 0x033B, this->actor.world.pos.x, this->actor.world.pos.y,
                    this->actor.world.pos.z, 0, 0, 0, 0x01);
        //SfxSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_IT_BIG_BOMB_EXPLOSION);
        SoundEffect_PlayOneshot(SOUND_POWDER_KEG_EXPLODE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor);
    }
...
void Wart_CheckDamage(Boss04* this, PlayState* play) {
    ...
    if ((this->unk_1FE == 0) && (this->collider1.elements[0].info.bumperFlags & BUMP_HIT)) {
        ...
        //Actor_PlaySfx(&this->actor, NA_SE_EN_ME_DAMAGE);
            SoundEffect_PlayOneshot(SOUND_ENEMY_WART_DAMAGE, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor);
...
void Wart_UpdateMatrix(Boss04* this) {
    s32 i;
...
void Wart_Update(Actor* thisx, PlayState* play2) {
    ...
    if (this->unk_74A != 0) {
        ...
        SoundEffect_PlayHeld(SOUND_ENEMY_WART_EXIST, 1.0f, 1.0f, 1.0f, &this->actor.world.pos, 0.0f, 2000.0f, &this->actor); 
    }
}

s32 Wart_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    Boss04* this = THIS;
...
void Wart_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == WART_LIMB_ROOT) {
        ...
        Wart_UpdateColliderProperty(0, &this->collider1, &sp18);
        Matrix_MultVec3f(&D_809EE234, &sp18);
        Wart_UpdateColliderProperty(0, &this->collider2, &sp18);
    }
}
...
void Wart_Draw(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          Wart_OverrideLimbDraw, Wart_PostLimbDraw, &this->actor);

    POLY_OPA_DISP = Play_SetFog(play, POLY_OPA_DISP);

    if (this->actionFunc != Wart_PlayCutscene) {
