...
static BgHidanCurtainParams sHCParams[] = { { 81, 144, 0.090f, 144.0f, 10.0f }/*5.0f*/, {46, 88, 0.055f, 88.0f, 10.0f}/*3.0f*/ };
...
void BgHidanCurtain_WaitForSwitchOn(BgHidanCurtain* this, PlayState* play) {
    if (Flags_GetSwitch(play, this->actor.params)) {
        if (this->type == 1) {
            ...
            this->timer = 5;    //The original value is 50.
        } else if (this->type == 3) {
            this->actionFunc = BgHidanCurtain_WaitForCutscene;
            //OnePointCutscene_Init(play, 3360, 60, &this->actor, CAM_ID_MAIN);
            this->timer = 5;    //The original value is 30.
        } else {
...
void BgHidanCurtain_TurnOff(BgHidanCurtain* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->actor.world.pos.y, this->actor.home.pos.y - hcParams->riseDist, hcParams->riseSpeed)) {
        if ((this->type == 0) /* || (this->type == 6)*/) { 
            //Remove the type 6 condition so that type 6 can be turned on or off by a toggle switch.
            Actor_Kill(&this->actor);
        } else if (this->type == 5) {
...
void BgHidanCurtain_Update(Actor* thisx, PlayState* play2) {
    ...
    if ((play->cameraPtrs[CAM_ID_MAIN]->setting == CAM_SET_SLOW_CHEST_CS) ||
        ...
        if (this->collider.base.atFlags & AT_HIT) {
            ...
            //Add a condition to check if the actor who gets hurt is Link, when the condition is true, knock the player out.
            //If you allow the circle of flame to damage the enemy, then this condition must be added.
            if(&player->actor == this->collider.base.at){
                //Knock out the player, Its damage type is determined by attack collider of this actor.
