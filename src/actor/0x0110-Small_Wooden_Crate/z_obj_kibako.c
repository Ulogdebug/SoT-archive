 ...
 * XX00 - XX = Switch Flag for conveyor (8/8 bits 1111111) FF= Ignore Switch Flags
 * 00YY - YY = Collectible Item (5/8 bits 00011111)
 */
...
void ObjKibako_Conveyor(ObjKibako* this, PlayState* play);
...
void ObjKibako_Init(Actor* thisx, PlayState* play) {
    ...
    //this->switchFlag = (this->actor.params & 0x3F00) >> 8;
    // "wooden box"
...
void ObjKibako_Idle(ObjKibako* this, PlayState* play) {
    ...
    if (Actor_HasParent(&this->actor, play)) {
        ...
        if (Flags_GetSwitch(play, 0X25)){
            ObjKibako_Conveyor(this, play);
        }
...
void ObjKibako_Conveyor(ObjKibako* this, PlayState* play) {
    ...
    static f32 sFloorConveyorSpeeds[3] = {
    ...
    conveyorSpeed = SurfaceType_GetConveyorSpeed(&play->colCtx, this->actor.floorPoly, this->actor.floorBgId);
    conveyorDir = SurfaceType_GetConveyorDirection(&play->colCtx, this->actor.floorPoly, this->actor.floorBgId);
    conveyorDir = CONVEYOR_DIRECTION_TO_BINANG(conveyorDir);

    if (conveyorSpeed != 0 && conveyorSpeed < 4){
        this->actor.world.rot.y = conveyorDir;
        Math_SmoothStepToF(&this->actor.speedXZ, sFloorConveyorSpeeds[conveyorSpeed-1], 0.7f, 0.5f, 0.0f);
        Actor_MoveForward(&this->actor);
    }else{
        if (DECR(offtimer) > 0){
            Math_SmoothStepToF(&this->actor.speedXZ, 0, 0.3f, 0.2f, 0.0f);
        }else{
            this->actor.world.rot.y = this->actor.shape.rot.y;
            Math_SmoothStepToF(&this->actor.speedXZ, 0, 0.7f, 0.5f, 0.0f);
        }
    }

    Debug_Text(255,255,255,2,10,"%d Speed",conveyorSpeed);
    Debug_Text(255,255,255,2,11,"%d Dir",conveyorDir);
    Debug_Text(255,255,255,2,12,"%d Rot",this->actor.world.rot.y);
    Debug_Text(255,255,255,2,13,"%d RotShape",this->actor.shape.rot.y);
