	...
	if (SkelAnime_Update(&this->skelAnime)) {
		if (this->despawntimer == 0) {
			...
			if (this->alpha != 0) {
				...
				if(!Actor_FindNearby(play, &this->actor, 0x0223, ACTORCAT_ENEMY, 8000.0f)){
...
void EnBoB_Init(EnBoB* this, PlayState* play) {
	...
	if ((this->switchFlag != 0x00) && Flags_GetSwitch(play, this->switchFlag) && (this->switchFlag != 0x3F))
    Actor_Kill(&this->actor);
