...
void EnIceHono_SetupActionCapturableFlame(EnIceHono* this, PlayState* play);
...
void EnIceHono_InitCapturableFlame(Actor* thisx, PlayState* play) {
    ...
    Actor_SetScale(&this->actor, 0.00008f);
    this->alpha = 0;
    ...
    if((Actor_FindNearby(play, &this->actor, 0x0206, ACTORCAT_BG, 50.0f) != NULL) || (play->sceneId == 0x6) ){
        this->superBonfireisNear = 1;
        Actor_Spawn(&play->actorCtx, play, 1, this->actor.world.pos.x, this->actor.world.pos.y, this->actor.world.pos.z, 0, 0, 0, 0x0000);
    }else
    {
        this->superBonfireisNear = 0;
    }
     this->actionFunc = EnIceHono_SetupActionCapturableFlame;
}
...
void EnIceHono_SetupActionCapturableFlame(EnIceHono* this,PlayState* play) {
   ...
   if((this->actor.scale.y >=0.0030f)){
    //func_8002F8F0(&this->actor, NA_SE_EV_FIRE_PILLAR_S - SFX_FLAG);
    Audio_PlayActorSfx2(&this->actor, NA_SE_EV_TORCH - SFX_FLAG);
   }
   if( (this->actor.scale.y >=0.0074f)/*&&(this->alpha>=250)*/){
    this->alpha = 255;
    this->actor.scale.x = this->actor.scale.z = this->actor.scale.y = 0.0074f;
    this->actionFunc = EnIceHono_CapturableFlame;
   }
   else{
     this->alpha = this->alpha + 51;
   this->actor.scale.y = this->actor.scale.y + 0.0001f;
...
void EnIceHono_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.params == -1) || (this->actor.params == 0)) {
        ...
        intensity = (Rand_ZeroOne() * 0.05f) + ((sin154 * 0.125f) + (sin156 * 0.1f)) + 0.825f;
        if ((intensity > 0.7f) || (intensity < 0.2f)) {
            osSyncPrintf("ありえない値(ratio = %f)\n", intensity); // "impossible value(ratio = %f)"
        }
        if(this->superBonfireisNear == 1){
        Lights_PointGlowSetInfo(&this->lightInfo, this->actor.world.pos.x, (s16)this->actor.world.pos.y + 10,
                                  this->actor.world.pos.z, (s32)(253.0f * intensity), (s32)(225.0f * intensity),
                                  (s32)(2.0f * intensity), 30000.0f * scale);
        }
            //    Lights_PointGlowSetInfo(&this->lightInfo, this->actor.world.pos.x, (s16)this->actor.world.pos.y + 10,
            //                       this->actor.world.pos.z, (s32)(155.0f * intensity), (s32)(210.0f * intensity),
            //                       (s32)(255.0f * intensity), 10000.0f * scale);                           
    }
...
void EnIceHono_Draw(Actor* thisx, PlayState* play) {
    ...
    if(this->superBonfireisNear == 1){
        gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, 255, 180, 100, this->alpha);
