...
#define FLAGS (ACTOR_FLAG_0 | ACTOR_FLAG_2 | ACTOR_FLAG_NO_UPDATE_CULLING | ACTOR_FLAG_5 | ACTOR_FLAG_10)
...
void EnRr_SetupStunnedAndKick(EnRr* this,PlayState* play);

void EnRr_InitBodySegments(EnRr* this, PlayState* play);

void EnRr_SetupDamage(EnRr* this, PlayState* play);
...
void EnRr_KickPlayer(EnRr* this, PlayState* play);
...
static DamageTable sDamageTable = {
    ...
    /* Explosive     */ DMG_ENTRY(0, RR_DMG_STUN),
    ...
    /* Light arrow   */ DMG_ENTRY(0, RR_DMG_STUN),
    ...
    /* Ice magic     */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Light magic   */ DMG_ENTRY(0, RR_DMG_STUN),
    ...
    /* Giant jump    */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Master jump   */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Unknown 1     */ DMG_ENTRY(0, RR_DMG_STUN),
...
void EnRr_SetupGrabPlayer(EnRr* this, Player* player) {
    ...
    this->grabTimer = 30; //the timer decides when your gears is comsumed
...
void EnRr_SetupReleasePlayer(EnRr* this, PlayState* play) {
    ...
    //    }
    //}
    //if ((CUR_EQUIP_VALUE(EQUIP_TYPE_TUNIC) != EQUIP_VALUE_TUNIC_KOKIRI)&& (this->grabTimer<= 40)) {
    ...
    if (this->actor.colorFilterTimer == 0) {
        ...
        if(!Flags_GetSwitch(play,0x3A)){
            Message_StartTextbox(play, 0x710D, NULL);
        }else if(this->kickTimer == 0 && this->yellFlag){
            this->cutsceneTimer = 60;
            Message_StartTextbox(play, 0x011E, NULL);
             this->actionFunc = EnRr_KickPlayer;
             return;
        }
        this->actionFunc = EnRr_SetupReach;
        Audio_PlayActorSfx2(&this->actor, NA_SE_EN_LIKE_THROW);
    } else if (this->actor.colChkInfo.health != 0) {
        EnRr_SetupDamage(this,play);
    } else {
...
void EnRr_SetupDamage(EnRr* this, PlayState* play) {
    ...
    if(!this->yellFlag){
        this->cutsceneTimer = 60;
        Message_StartTextbox(play, 0x716A, NULL);
        this->actionFunc = EnRr_KickPlayer;
        return;
    }else{
        Message_StartTextbox(play, 0x7068, NULL);
    }
...
void EnRr_KickPlayer(EnRr* this, PlayState* play) {
    ...
    if (this->cutsceneTimer > 0) {
        ...
        func_8006D074(play);
        // play->nextEntranceIndex = ENTR_HYLIA_LABO_1;
        // play->transitionType = TRANS_TYPE_CIRCLE(TCA_STARBURST, TCC_BLACK, TCS_FAST);
        // play->transitionTrigger = TRANS_TRIGGER_START;

        gExitParam.exit.value = 0;
        gExitParam.exit.sceneIndex = 0x0C;
        ...
        if (this->collider1.base.acFlags & AC_HIT) {
            ...
            switch (this->actor.colChkInfo.damageEffect) {
                ...
                case RR_DMG_NORMAL:
                    ...
                    this->stopScroll = false;
                    if(this->yellFlag && this->cutsceneTimer >= 0 ){
                ...
                case RR_DMG_FIRE: // Fire Arrow and Din's Fire
                    ...
                    this->effectTimer = 20;
                    EnRr_SetupStunnedAndKick(this,play);
                    return;
                ...
                case RR_DMG_STUN: // Boomerang and Hookshot
                    ...
                    Actor_SetColorFilter(&this->actor, -0x8000, 0xFF, 0x2000, 0x1C);//the original were 0xFF  0x2000 0x50
                    EnRr_SetupStunnedAndKick(this,play);
                    return;
            }
        }
...
void EnRr_Approach(EnRr* this, PlayState* play) {
    Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xA, 0x3E8, 0);// the original were 0xA, 0x1F4, 0  */  
    ...
    if ((this->actionTimer == 0) && (this->actor.xzDistToPlayer < 160.0f)) {
        ...
        EnRr_SetSpeed(this, 0.1f);//original 2.0f
    }
}
        ...
        case REACH_GAPE:
            if (this->actionTimer == 0) {
                if ((this->actor.xzDistToPlayer >  120.0f) && (this->actor.xzDistToPlayer <= 180.0f))
                {
                    ...
                    this->actionTimer = 0;//the original was 20
                }
...
void EnRr_Retreat(EnRr* this, PlayState* play) {
    if (this->actionTimer == 0) {
        ...
        if (this->actor.speedXZ == 0.0f) {
            EnRr_SetSpeed(this, 0.1f);//original speed 2.0f
        }
    }
}
...
void EnRr_Update(Actor* thisx, PlayState* play) {
    ...
    if(Flags_GetSwitch(play,0x3A)){
        if(!this->yellFlag){
            this->yellFlag = 1;
            this->kickTimer = 200;
            Message_StartTextbox(play, 0x4044, NULL);
        }else{
            DECR(this->kickTimer);
        }
