 ...
 */
                                                 /*  CUSTOM   */
//If a torch is placed in a circle with a radius of 50m around the push block, it will be automatically attached to the top of the push block.
#include <uLib.h>
...
void ObjOshihiki_TorchPosUpdate(ObjOshihiki* thisx, PlayState* play);
...
static f32 torchHeight[] = {
    40.0f,80.0f,100.0f,180.0f,
};
...
void ObjOshihiki_Init(Actor* thisx, PlayState* play2) {
    ...
    s16 flags = (this->dyna.actor.params >> 8) & 0xFF;
    // if(this->dyna.actor.world.rot.z != 0){
    //     this->dyna.actor.world.rot.z = 0;
    //     this->dyna.actor.category = ACTORCAT_BG;
    // }
    ObjOshihiki_CheckType(this, play);
    this->searchTorchTimer = 20;
...
void ObjOshihiki_TorchPosUpdate(ObjOshihiki* this, PlayState* play){
        this->torchPos->world.pos.x = this->dyna.actor.world.pos.x;
        this->torchPos->world.pos.z = this->dyna.actor.world.pos.z;
        this->torchPos->world.pos.y = this->dyna.actor.world.pos.y + torchHeight[this->dyna.actor.params & 0xF];
}
...
void ObjOshihiki_Push(ObjOshihiki* this, PlayState* play) {
    ...
    this->pushSpeed += 1.5f;    //original value is 0.5f, super fast huh?
    this->stateFlags |= PUSHBLOCK_PUSH;
    this->pushSpeed = CLAMP_MAX(this->pushSpeed, 5.0f); //original value is 2.0f, control the maximum, don't let Link be able to fly
    ...
    if (!ObjOshihiki_CheckFloor(this, play)) {
        ...
        this->pushDist = 0.0f;
        this->pushSpeed = 0.0f;
        this->timer = 0;    //original value is 10.
        if (this->floorBgIds[this->highestFloor] == BGCHECK_SCENE) {
            ObjOshihiki_SetupOnScene(this, play);
        } else {
            ObjOshihiki_SetupOnActor(this, play);
...
void ObjOshihiki_Update(Actor* thisx, PlayState* play) {
    ...
    if((this->dyna.actor.params & 0xF) == PUSHBLOCK_MEDIUM_START_ON){
        Debug_Text(255,255,255,1,1,"X:%d,Y:%d,Z:%d",this->dyna.actor.world.pos.x,this->dyna.actor.world.pos.y,this->dyna.actor.world.pos.z);
    }
    if (this->timer > 0) {
        this->timer--;
    }

    if((this->searchTorchTimer > 0) && (this->hasTorch == false)){
        //Search all torch actor within a 50.0m radius for 20 frames
         if(Actor_FindNearby(play, &this->dyna.actor, ACTOR_OBJ_SYOKUDAI, ACTORCAT_PROP, searchRadius) != NULL){ 
             this->torchPos = Actor_FindNearby(play, &this->dyna.actor, ACTOR_OBJ_SYOKUDAI, ACTORCAT_PROP, searchRadius);
             this->hasTorch = true;
             this->torchPos->world.pos.x = this->dyna.actor.world.pos.x;
             this->torchPos->world.pos.z = this->dyna.actor.world.pos.z;
             this->torchPos->world.pos.y = this->dyna.actor.world.pos.y + torchHeight[this->dyna.actor.params & 0xF];
             ...
             this->searchTorchTimer--;
         }
    }
    else if(this->hasTorch == true){
        ObjOshihiki_TorchPosUpdate(this,play);
    }
...
void ObjOshihiki_Draw(Actor* thisx, PlayState* play) {
    ...
    switch (play->sceneId) {
        ...
        default:
            ...
            gDPSetEnvColor(POLY_OPA_DISP++, this->color.r, this->color.g, this->color.b, 255); //In case it becomes total black
            break;
    }
