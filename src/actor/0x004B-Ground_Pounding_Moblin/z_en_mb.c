...
typedef enum {
    /* 2 */ ENMB_TYPE_SPEAR_GUARD = 0,
    /*  0 */ ENMB_TYPE_CLUB = 1,
    /*  1 */ ENMB_TYPE_SPEAR_PATROL = 2
} EnMbType;
...
static DamageTable sSpearMoblinDamageTable = {
    /* Deku nut      */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
    ...
    /* Normal arrow  */ DMG_ENTRY(1, ENMB_DMGEFF_DEFAULT),
    /* Hammer swing  */ DMG_ENTRY(4, ENMB_DMGEFF_DEFAULT),
    ...
    /* Fire arrow    */ DMG_ENTRY(4, ENMB_DMGEFF_DEFAULT),
    ...
    /* Unblockable   */ DMG_ENTRY(2, ENMB_DMGEFF_DEFAULT),
    /* Hammer jump   */ DMG_ENTRY(8, ENMB_DMGEFF_DEFAULT),
    /* Unknown 2     */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
};

static DamageTable sClubMoblinDamageTable = {
    /* Deku nut      */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
    /* Deku stick    */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
    /* Slingshot     */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
    /* Explosive     */ DMG_ENTRY(4, ENMB_DMGEFF_DEFAULT),
    /* Boomerang     */ DMG_ENTRY(0, ENMB_DMGEFF_STUN),
    /* Normal arrow  */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
    /* Hammer swing  */ DMG_ENTRY(4, ENMB_DMGEFF_DEFAULT),
    /* Hookshot      */ DMG_ENTRY(0, ENMB_DMGEFF_STUN),
    /* Kokiri sword  */ DMG_ENTRY(1, ENMB_DMGEFF_DEFAULT),
    /* Master sword  */ DMG_ENTRY(2, ENMB_DMGEFF_DEFAULT),
    ...
    /* Master jump   */ DMG_ENTRY(4, ENMB_DMGEFF_DEFAULT),
    /* Unknown 1     */ DMG_ENTRY(0, ENMB_DMGEFF_FREEZE),
    /* Unblockable   */ DMG_ENTRY(0, ENMB_DMGEFF_STUN),
    /* Hammer jump   */ DMG_ENTRY(8, ENMB_DMGEFF_DEFAULT),
    /* Unknown 2     */ DMG_ENTRY(0, ENMB_DMGEFF_IGNORE),
};
...
void EnMb_Init(Actor* thisx, PlayState* play) {
    ...
    this->moblinType = this->actor.params & 0xF;
    ...
    this->switchFlags = (thisx->params & 0xFF00) >> 8;

    switch (this->moblinType) {
        case ENMB_TYPE_SPEAR_GUARD:
            ...
            this->playerDetectionRange = 700.0f;
            Actor_SetScale(&this->actor, 0.011f);
            this->attackCollider.info.toucher.dmgFlags = 0xFFCFFFFF;
        ...
        default: /* Spear Patrol */
            ...
            this->moblinType = ENMB_TYPE_SPEAR_PATROL;
            this->waypoint = 0;
...
void EnMb_SetupSpearGuardWalk(EnMb* this) {
    Animation_Change(&this->skelAnime, &gEnMbSpearWalkAnim, 1.5f, 0.0f, Animation_GetLastFrame(&gEnMbSpearWalkAnim),
                     ANIMMODE_LOOP, 6.0f);
...
void EnMb_SetupSpearPrepareAndCharge(EnMb* this) {
    ...
    this->actor.speedXZ = 0.0f;
    this->timer3 = (s16)frameCount + 1;
    Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0x9EE, 0);
    Audio_PlayActorSfx2(&this->actor, NA_SE_EN_MORIBLIN_SPEAR_AT);
    if (this->moblinType == ENMB_TYPE_SPEAR_GUARD) {
...
void EnMb_SetupStunned(EnMb* this) {
    ...
    this->actor.speedXZ = 0.0f;
    Actor_SetColorFilter(&this->actor, 0, 0x78, 0, 0x25);
    if (this->damageEffect == ENMB_DMGEFF_STUN_ICE) {
        this->iceEffectTimer = 40;
    } else {
        if (this->moblinType != ENMB_TYPE_CLUB) {
...
void EnMb_SpearGuardLookAround(EnMb* this, PlayState* play) {
    ...
    || (Math_Vec3f_DistXZ(&this->actor.world.pos, &player->actor.world.pos) < this->playerDetectionRange) ) {
        EnMb_SetupSpearGuardWalk(this);
    }
}
...
void EnMb_SpearGuardPrepareAndCharge(EnMb* this, PlayState* play) {
    ...
    if (this->timer3 != 0) {
        this->timer3--;
        if (((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) && (ABS(temp_a1) >= 0x4000))||(ABS(relYawFromPlayer) >= 0x3E80)) {
            temp_a1 = (this->actor.yawTowardsPlayer > 0) ? this->actor.wallYaw - 0x4000 : this->actor.wallYaw + 0x4000;
            Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, 0xEB8, 0);
        }else{
        ...
        this->attack = ENMB_ATTACK_SPEAR;
        Math_ApproachF(&this->actor.speedXZ, 11.0f, 0.6f, 0.5f);
        Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, 5.0f, 3, 4.0f, 100, 15, false);
        ...
        if(this->actor.speedXZ >= unstoppableSpeed){
            Audio_PlayActorSfx2(&this->actor, NA_SE_EN_MORIBLIN_VOICE);
            this->attackCollider.info.toucher.dmgFlags = DMG_UNBLOCKABLE;
            if (relYawTowardsPlayerAbs > 0x1388 && ((this->actor.xzDistToPlayer >= 80.0f) || (ABS(this->actor.yDistToPlayer) >= 45.0f)) || (this->actor.bgCheckFlags & BGCHECKFLAG_WALL)){
                this->attack = ENMB_ATTACK_NONE;
                EnMb_SetupSpearEndChargeQuick(this);
            }
        }else{
            this->actor.world.rot.y = this->actor.shape.rot.y;
            Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 2, 0x108, 0);
            this->attackCollider.info.toucher.dmgFlags = 0xFFCFFFFF;
        }        
    }
    ...
    if (this->attackCollider.base.atFlags & AT_HIT) {
        ...
        if (this->attackCollider.base.at == &player->actor) {
            ...
            if(this->actor.speedXZ >= unstoppableSpeed){
                func_8002F71C(play, &this->actor, CLAMP(this->actor.speedXZ - 2.0f,3.0f,6.0f),
                          this->actor.world.rot.y, CLAMP(this->actor.speedXZ - 2.0f,4.0f,6.0f));
                 this->attack = ENMB_ATTACK_NONE;
                EnMb_SetupSpearEndChargeQuick(this);
            }
...
void EnMb_ClubAttack(EnMb* this, PlayState* play) {
    ...
    s16 turnSpeed;
    if(Actor_WorldDistXYZToPoint(&player->actor, &this->actor.world.pos) <= 200.0f){
        turnSpeed = 0x250;
        if(ABS(relYawFromPlayer) < 0x3E80
        && Actor_WorldDistXYZToPoint(&player->actor, &this->actor.world.pos) <= 100.0f){
            turnSpeed = 0x6FF;
        }
    }else{
        turnSpeed = 0x180;
    ...
    Math_SmoothStepToS(&this->actor.shape.rot.y, 
        /*relYawTarget[this->attack - 1] + this->actor.world.rot.y*/ this->actor.yawTowardsPlayer, 
        1,
        turnSpeed, 
        0
    );
    this->actor.world.rot.y = this->actor.shape.rot.y;
    if(Actor_WorldDistXYZToPoint(&player->actor, &this->actor.world.pos) <= 150.0f){
        this->attackCollider.info.toucher.damage = 16;
    }else{
        this->attackCollider.info.toucher.damage = 8;
    }
    if (this->attackCollider.base.atFlags & AT_HIT) {
        this->attackCollider.base.atFlags &= ~AT_HIT;
        if (this->attackCollider.base.at == &player->actor) {
            ...
            if (player->invincibilityTimer < 0) {
             ...
             if(Actor_WorldDistXYZToPoint(&player->actor, &this->actor.world.pos) <= 150.0f){
                 func_8002F71C(play, &this->actor, 2.0f,
                          this->actor.world.rot.y, 10.0f);
             }else{
                 func_8002F71C(play, &this->actor, 2.0f,
                          this->actor.world.rot.y, 3.0f);
             }
...
void EnMb_SpearGuardWalk(EnMb* this, PlayState* play) {
    ...
    Math_SmoothStepToF(&this->actor.speedXZ, 0.8f, 0.1f, 1.0f, 0.0f);
    ...
    playSpeedAbs = ABS(this->skelAnime.playSpeed);
    if (/*(Math_Vec3f_DistXZ(&this->actor.home.pos, &player->actor.world.pos) < this->playerDetectionRange) && */this->searchTimer > 0)
    {
        Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0x6EE, 0);
        this->actor.flags |= ACTOR_FLAG_0;
        ...
        this->actor.flags &= ~ACTOR_FLAG_0;
        if (Math_Vec3f_DistXZ(&this->actor.world.pos, &this->actor.home.pos) > this->maxHomeDist || this->timer2 != 0 || ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) && (ABS(yawToWall) >= 0x4000))) {
            yawTowardsHome = Math_Vec3f_Yaw(&this->actor.world.pos, &this->actor.home.pos);
            Math_SmoothStepToS(&this->actor.world.rot.y, yawTowardsHome, 1, 0x8EE, 0);
            this->actor.shape.rot.y = this->actor.world.rot.y;
        }
        if (this->timer2 != 0) {
            this->timer2--;
...
void EnMb_ClubWaitPlayerNear(EnMb* this, PlayState* play) {
    ...
    SkelAnime_Update(&this->skelAnime);
    if((this->actor.yDistToPlayer <= 45.0f) && (this->actor.yDistToPlayer >= -45.0f)){
        if (((Math_Vec3f_DistXZ(&this->actor.world.pos, &player->actor.world.pos) < this->playerDetectionRange) &&
        (!(player->stateFlags1 & PLAYER_STATE1_26)) && (ABS(relYawFromPlayer) < 0x3E80) && (this->actor.yDistToPlayer <= 60.0f))
        || (func_8002DDE4(play) && Math_Vec3f_DistXZ(&this->actor.world.pos, &player->actor.world.pos) <= (this->playerDetectionRange * 0.5f)) ) {
...
void EnMb_SetupSpearDead(EnMb* this) {
    ...
    if (ABS(relYawTowardsPlayer) <= 0x4000) {
        ...
        Animation_MorphToPlayOnce(&this->skelAnime, &gEnMbSpearFallFaceDownAnim, -4.0f);
        this->actor.speedXZ = 8.0f;
    }
...
void EnMb_SpearUpdateAttackCollider(Actor* thisx, PlayState* play) {
    ...
    if (this->moblinType >= ENMB_TYPE_SPEAR_PATROL) {
...
void EnMb_CheckColliding(EnMb* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    if(this->actor.world.pos.y <= -1500.0f){
       if(this->switchFlags != 0x0 && this->switchFlags != 0x3F){
           Flags_SetSwitch(play, this->switchFlags);
           Actor_Kill(&this->actor);
       }
    ...
    if (this->frontShielding.base.acFlags & AC_HIT) {
        ...
        this->hitbox.base.acFlags &= ~AC_HIT;
    } else if ((this->hitbox.base.acFlags & AC_HIT) && this->state >= ENMB_STATE_CLUB_KNEELING_DAMAGED) {
        this->hitbox.base.acFlags &= ~AC_HIT;
        if (this->actor.colChkInfo.damageEffect != ENMB_DMGEFF_IGNORE &&
            ...
            if (this->actor.colChkInfo.damageEffect == ENMB_DMGEFF_STUN ||
                ...
                if (this->moblinType == ENMB_TYPE_CLUB) {
                    if (this->actor.colChkInfo.health == 0) {
                        EnMb_SetupClubDead(this);
                    } else if (this->state != ENMB_STATE_CLUB_DEAD) {
...
void EnMb_Update(Actor* thisx, PlayState* play) {
    EnMb* this = (EnMb*)thisx;
    s32 pad;
    if(this->actor.world.pos.y - this->actor.floorHeight>=600.0f){
        //Audio_PlayActorSfx2(&this->actor, NA_SE_EN_MORIBLIN_DEAD - SFX_FLAG);
    }
    EnMb_CheckColliding(this, play);
    if( thisx->colChkInfo.health > 0){
        if ((this->state >= ENMB_STATE_STUNNED) /*&&
            (this->moblinType == ENMB_TYPE_CLUB || this->state != ENMB_STATE_ATTACK)*/) {
    ...
    if (thisx->colChkInfo.damageEffect != ENMB_DMGEFF_FREEZE) {
        ...
        if(thisx->colorFilterTimer ==0 && thisx->colChkInfo.health > 0){
            if (this->state >= ENMB_STATE_IDLE && (thisx->colorFilterTimer ==0)) {
