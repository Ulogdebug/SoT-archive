...
void EnNiw_FleeTypeActionChoose(EnNiw* this, PlayState* play);
void EnNiw_NormalTypeActionChoose(EnNiw* this, PlayState* play);
void func_80AB6A38(EnNiw* this, PlayState* play);
void EnNiw_NormalTypeLifted(EnNiw* this, PlayState* play);
void EnNiw_PutDownByPlayer(EnNiw* this, PlayState* play);
...
void EnNiw_SetupFleeFromPlayer(EnNiw* this, PlayState* play);
void EnNiw_FleeFromPlayer(EnNiw* this, PlayState* play);
...
void EnNiw_SpawnAttackCucco(EnNiw* this, PlayState* play) {
    ...
    if ((this->readyToFleeTimer == 0) && (this->unk_296 < 7)) {
...
void func_80AB6100(EnNiw* this, PlayState* play, s32 arg2) {
    ...
    if (arg2 == 1) {
        if (this->fleeTimer == 0 || this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
            this->fleeTimer = 150;
            if (this->timer8 == 0) {
                this->timer8 = 70;
...
void EnNiw_ResetAction(EnNiw* this, PlayState* play) {
    ...
    switch (this->actor.params) {
        case 0x4: //Flee one
            this->actionFunc = EnNiw_FleeTypeActionChoose;
            break;
        case 0xD:
            this->actionFunc = func_80AB6324;
            break;
        default:
            this->actionFunc = EnNiw_NormalTypeActionChoose;
            break;
    }
}
...
void func_80AB63A8(EnNiw* this, PlayState* play) {
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && this->actor.velocity.y < 0.0f) {
        ...
        this->readyToFleeTimer = this->timer4 = this->unk_29E = 0;
...
void EnNiw_FleeTypeActionChoose(EnNiw* this, PlayState* play) {
    ...
    if (this->actor.xzDistToPlayer < 30.0f && fabsf(this->actor.world.pos.y - player->actor.world.pos.y) < 5.0f) {
        ...
        this->actionFunc = EnNiw_SetupFleeFromPlayer;
        ...
        this->actionFunc = EnNiw_NormalTypeLifted;
    } else {
...
void EnNiw_NormalTypeActionChoose(EnNiw* this, PlayState* play) {
    ...
    if (this->readyToFleeTimer == 0 && this->timer4 == 0) {
        this->unk_29E++;

        if (this->unk_29E >= 8) {
            this->readyToFleeTimer = Rand_ZeroFloat(30.0f);
...
void EnNiw_NormalTypeLifted(EnNiw* this, PlayState* play) {
    ...
    if (Actor_HasNoParent(&this->actor, play)) {
        ...
        this->actor.flags |= ACTOR_FLAG_0;
        this->actionFunc = EnNiw_PutDownByPlayer;
    }
    func_80AB5BF8(this, play, 2);
}
...
void EnNiw_SetupFleeFromPlayer(EnNiw* this, PlayState* play) {
    ...
    this->unk_2A0 = Rand_ZeroFloat(1.99f);
    this->actor.speedXZ = 4.0f;
    this->actionFunc = EnNiw_FleeFromPlayer;
}
...
void EnNiw_SetTimerAndReset(EnNiw* this, PlayState* play) {
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        this->unk_2A4 = (s16)Rand_ZeroFloat(3.99f) + 5;
        this->actionFunc = EnNiw_ResetAction;
    }
...
void EnNiw_ColliderCheck(EnNiw* this, PlayState* play) {
    if (this->cuccosAttackFlag == 0 && this->actor.params != 0xA && this->actionFunc != EnNiw_FleeTypeActionChoose &&
...
void EnNiw_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnNiw_NormalTypeActionChoose) {
        this->unk_26C[9] = 0.0f;
    }
    if (this->unk_2A6) {
        featherCount = 20;
    ...
    if (this->actionFunc != func_80AB6EB4 && this->actionFunc != EnNiw_FleeTypeActionChoose && play->sceneId != SCENE_SPOT03) {
    ...
    s32 surfaceType = SurfaceType_GetFloorType(&play->colCtx, this->actor.floorPoly, this->actor.floorBgId);
    if ((thisx->floorHeight <= BGCHECK_Y_MIN || thisx->floorHeight >= 32000.0f) || (surfaceType == FLOOR_TYPE_9 && this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_GROUND_TOUCH) )) {
        ...
        thisx->world.pos.x = thisx->home.pos.x; //"Respawn"
        ...
        this->unk_2DC = 0.0f;
        this->unk_2E0 = 0.0f;
        this->cuccosAttackFlag = this->unk_294 = this->unk_298 = this->unk_2A6 = this->unk_29E = this->unk_2A0 = this->unk_2A2 =
            0;
        ...
        this->cuccosAttackFlag = 0;
        this->actionFunc = EnNiw_SetTimerAndReset;
        return;
    }
    ...
    if (D_80AB85E0 == 0 && this->unk_2A4 <= 0 && thisx->params != 0xD && thisx->params != 0xE && thisx->params != 0xA &&thisx->params != 0x8 ) {
        this->fleeTimer = 100;
    ...
    if (this->cuccosAttackFlag != 0 && thisx->xyzDistToPlayerSq < SQ(dist) && player->invincibilityTimer == 0) {
        func_8002F6D4(play, &this->actor, 2.0f, thisx->world.rot.y, 0.0f, 0x10);
    }

    EnNiw_ColliderCheck(this, play);

    if (this->sfxTimer2 == 0 && this->actionFunc == EnNiw_NormalTypeLifted) {
        this->sfxTimer2 = 7;
    ...
    if (this->cuccosAttackFlag == 0) {
        ...
        if (this->actionFunc != EnNiw_NormalTypeLifted && this->actionFunc != EnNiw_PutDownByPlayer &&
...
void EnNiw_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc == EnNiw_FleeTypeActionChoose) {
