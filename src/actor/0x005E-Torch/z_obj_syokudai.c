...
static ColliderCylinderInit sCylInitFlame = {
    ...
    },
    {
        ...
        {  (DMG_FIRE |  DMG_ARROW_NORMAL) , 0x00, 0x00 },
        TOUCH_NONE,
        BUMP_ON,
        OCELEM_NONE,
    },
...
void ObjSyokudai_Update(Actor* thisx, PlayState* play2) {
    ...
    if (WaterBox_GetSurfaceImpl(play, &play->colCtx, this->actor.world.pos.x, this->actor.world.pos.z, &waterSurface,
                                &dummy) &&
        ((waterSurface - this->actor.world.pos.y) > 55.0f)) {
        Debug_Text(0,0,255,3,3,"overflow");
        this->litTimer = 0;
        /*if (torchType == 1) {*/
        ...
        if (this->colliderFlame.base.acFlags & AC_HIT) {
            ...
            tipToFlame.y -= 67.0f;
            if ((SQ(tipToFlame.x) + SQ(tipToFlame.y) + SQ(tipToFlame.z)) < SQ(25.0f)) {//20.0f
                interactionType = -1;
            }
        }
    ...
    if (this->actor.world.pos.y <= BGCHECK_Y_MIN + 10.0f) {
