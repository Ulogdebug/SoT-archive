...
static ColliderCylinderInit sCylinderInit = {
    .base = {
        ...
        .atFlags = AT_ON | AT_TYPE_ALL,
        .acFlags = AC_ON | AC_TYPE_PLAYER,
        .ocFlags1 = OC1_ON | OC1_NO_PUSH | OC1_TYPE_1 | OC1_TYPE_2,
        .ocFlags2 = OC2_TYPE_1,
        .shape = COLSHAPE_CYLINDER,
    },
    .info = {
        .elemType = ELEMTYPE_UNK0,
        .toucher = { 0x20000000, 0x00, 0x04 },
        .bumper = { DMG_DEFAULT, 0x00, 0x00 },
        .toucherFlags = TOUCH_ON,
        .bumperFlags = BUMP_ON,
        .ocElemFlags = OCELEM_ON
    },
    .dim = { .radius = 30, .height = 20 } ,
};
...
void EnTrap_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if(this->collider.info.acHitInfo->toucher.dmgFlags & DMG_ARROW_ICE){
            ...
            icePos.x -= 20.0f;
            EffectSsEnIce_SpawnFlyingVec3f(play, thisx, &icePos, 150, 150, 150, 250, 235, 245, 255, 1.8f);
        }else if(this->collider.info.acHitInfo->toucher.dmgFlags & DMG_HAMMER){
            Actor_SetColorFilter(thisx,-0x8000, 0xFF, 0, 150);
        }
    }else{
    ...
    if (thisx->colorFilterTimer == 0) {
        ...
        if ((this->collider.base.atFlags & AT_HIT) ) {
            ...
            if (&player->actor == this->collider.base.at){
                ...
                func_8002F7A0(play, thisx, this->knockedSpeed, angleToKnockPlayer, 6.0f);
                this->playerDmgTimer = 15;
            }
        }
        // if ((Actor_FindNearby(play, &this->actor, ACTOR_OBJ_KIBAKO, ACTORCAT_PROP, 40.0f) != NULL)) {

        //     this->crateActor = Actor_FindNearby(play, &this->actor, ACTOR_OBJ_KIBAKO, ACTORCAT_PROP, 40.0f);
        //     ObjKibako* thisCrate = (ObjKibako*)this->crateActor;
        //     Actor_Kill(this->crateActor);
        //     if (this->crateActor != NULL) {
        //         this->crateActor = NULL;
        //     }
        // }
...
void EnTrap_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->crateActor != NULL) {
        GfxPrint_Printf(&printer, "pointer = %x", this->crateActor);
    }
    else {
        GfxPrint_Printf(&printer, "pointer = Null%x", this->crateActor);
    }
