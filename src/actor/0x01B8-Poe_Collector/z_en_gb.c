...
void En_Gb_ItemCheck(EnGb* this, PlayState* play);
void func_80A2FC70(EnGb* this, PlayState* play);
void En_Gb_TradeBigPoe(EnGb* this, PlayState* play);
void En_Gb_TradeNormalPoe(EnGb* this, PlayState* play);
void func_80A2F94C(EnGb* this, PlayState* play);
void En_Gb_SetupGivePrize(EnGb* this, PlayState* play);
void En_Gb_GivePrize(EnGb* this, PlayState* play);
...
void En_Gb_TalkBegin(EnGb* this) {
    if (!GET_INFTABLE(INFTABLE_B6)) {//second talk
        this->textId = 0x70F4;
    } else if(CustomFlag_Get(0x15)){
        this->textId = 0x0421;
    }else if(!(HIGH_SCORE(HS_POE_POINTS) >= 1000 && CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER))){
        this->textId = 0x70F5;
    }else if ((HIGH_SCORE(HS_POE_POINTS) == 2000 && CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER))){
        this->textId = 0x0421; //kill all of the poes
    }else if((HIGH_SCORE(HS_POE_POINTS) < 2000 && CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER))){
        this->textId = 0x1013;
    }else{
        this->textId = 0x710f;
...
void EnGb_Init(Actor* thisx, PlayState* play) {
    ...
    this->switchFlags = 0xF; //set if you get prize
    //HIGH_SCORE(HS_POE_POINTS) = 2000;
    En_Gb_TalkBegin(this);
    this->actionFunc = En_Gb_ItemCheck;
    // if (HIGH_SCORE(HS_POE_POINTS) >= 2000 && CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER)) {//if you have finished the quest
    //     Actor_Kill(thisx);
    // }
}
...
s32 En_Gb_PoeEscapeCheck(EnGb* this) {
    s32 i;
...
void En_Gb_KickCagedPoe(EnGb* this) {
...
void En_Gb_ItemCheck(EnGb* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
     ...
     if (!En_Gb_PoeEscapeCheck(this)) {
         if (this->actionTimer != 0) {
             this->actionTimer--;
         } else {
             En_Gb_KickCagedPoe(this);
             return;
         }
     }
        if (Actor_ProcessTalkRequest(&this->dyna.actor, play)) {
            switch (func_8002F368(play)) { //func_8002F368 send the name of item player delivers currently
            case EXCH_ITEM_NONE:
                En_Gb_TalkBegin(this);
            ...
            case EXCH_ITEM_POE:
                ...
                this->actionFunc = En_Gb_TradeNormalPoe;
                break;
            case EXCH_ITEM_BIG_POE:
                player->actor.textId = 0x70F7;
                this->actionFunc = En_Gb_TradeBigPoe;
                break;
            }
            return;
        }
...
void func_80A2F94C(EnGb* this, PlayState* play) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_DONE && Message_ShouldAdvance(play)) {
        ...
        En_Gb_TalkBegin(this);
        if (HIGH_SCORE(HS_POE_POINTS) >= 1000 && (!CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER)) ) {        
            Player* player = GET_PLAYER(play);
            player->exchangeItemId = EXCH_ITEM_NONE;
            this->textId = 0x70F8;
            Message_ContinueTextbox(play, this->textId);
            this->actionFunc = En_Gb_SetupGivePrize;
        }else if (HIGH_SCORE(HS_POE_POINTS) == 2000 && (!CustomFlag_Get(0x15))){
            Player* player = GET_PLAYER(play);
            player->exchangeItemId = EXCH_ITEM_NONE;
            this->textId = 0x713C;
            Message_ContinueTextbox(play, this->textId);
...
void En_Gb_TradeNormalPoe(EnGb* this, PlayState* play) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_DONE && Message_ShouldAdvance(play)) {
       ...
       En_Gb_TalkBegin(this);
        ...
        Rupees_ChangeBy(20);
        if (HIGH_SCORE(HS_POE_POINTS) < 1000 || (Flags_GetSwitch(play, this->switchFlags)) ) {           
            this->actionFunc = En_Gb_ItemCheck;
        }else {//if 1000, tell you that you have satisfy the need
            Player* player = GET_PLAYER(play);
            player->exchangeItemId = EXCH_ITEM_NONE;
            this->textId = 0x70F8;
            Message_ContinueTextbox(play, this->textId);
...
void En_Gb_SetupGivePrize(EnGb* this, PlayState* play) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_DONE && Message_ShouldAdvance(play)) {
        if(!CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER)){
            func_8002F434(&this->dyna.actor, play, GI_BOOTS_HOVER, 100.0f, 10.0f);// the function give you items
        }else if(HIGH_SCORE(HS_POE_POINTS) == 2000){
            func_8002F434(&this->dyna.actor, play, GI_HEART_PIECE, 100.0f, 10.0f);
            if(!CustomFlag_Get(0x15)){
                CustomFlag_Set(0x15,1);
            }

        }
        this->actionFunc = En_Gb_GivePrize;
    }
}
...
void En_Gb_GivePrize(EnGb* this, PlayState* play) {
    ...
    if (Actor_HasParent(&this->dyna.actor, play)) {
        ...
        if(!CHECK_OWNED_EQUIP(EQUIP_TYPE_BOOTS, EQUIP_INV_BOOTS_HOVER)){
            func_8002F434(&this->dyna.actor, play, GI_BOOTS_HOVER, 100.0f, 10.0f);
        }else if(HIGH_SCORE(HS_POE_POINTS) == 2000){
            func_8002F434(&this->dyna.actor, play, GI_HEART_PIECE, 100.0f, 10.0f);
        }
    }
