...
static DamageTable sDamageTable = {
    ...
    /* Deku stick    */ DMG_ENTRY(1, REDEAD_DMGEFF_DAMAGE),
    ...
    /* Fire magic    */ DMG_ENTRY(6, REDEAD_DMGEFF_FIRE_MAGIC),
    ...
    /* Giant spin    */ DMG_ENTRY(2, REDEAD_DMGEFF_DAMAGE),
    /* Master spin   */ DMG_ENTRY(1, REDEAD_DMGEFF_DAMAGE),
...
void EnRd_Idle(EnRd* this, PlayState* play) {
    ...
    if (this->actor.parent != NULL) {
        ...
        if (this->actor.params <= REDEAD_TYPE_GIBDO) {
            if (Actor_IsFacingPlayer(&this->actor, 60 * 0x10000 / 360)){
                ...
                if ((this->actor.xzDistToPlayer <= 100.0f) && func_8002DDE4(play)&& (ABS(this->actor.yDistToPlayer <= 45.0f))) {
                    if ((this->actor.params != REDEAD_TYPE_CRYING) && !this->isMourning) {
                        EnRd_SetupAttemptPlayerFreeze(this);
                    } else {
                        EnRd_SetupStandUp(this);
    ...
    if ((play->gameplayFrames & 0x5F) == 0 && (this->actor.xyzDistToPlayerSq < SQ(800.0f))) {
...
void EnRd_Grab(EnRd* this, PlayState* play) {
    ...
    switch (this->grabState) {
        ...
        case REDEAD_GRAB_ATTACK:
            ...
            }else if (this->forwardOffset != -25.0f){
                Math_SmoothStepToF(&this->actor.shape.yOffset, -200.0f, 1.0f, 150.0f, 0.0f);
            }
...
void EnRd_SetupResurrect(EnRd* this) {
    ...
    Audio_PlayActorSfx2(&this->actor, NA_SE_EN_REDEAD_CRY);
    Animation_Change(&this->skelAnime, &gGibdoRedeadDeathAnim,  -0.8f, this->skelAnime.curFrame, 0.0f, ANIMMODE_ONCE, -6.0f);
...
void EnRd_SetupDead(EnRd* this) {
    ...
    if(this->realDead == 1){
        this->fireTimer = 250;
        this->timer = 250;//original was 300
    }
    else {
        this->timer = 550;
    }
            ...
            if ((this->damageEffect != REDEAD_DMGEFF_NONE) && (this->damageEffect != REDEAD_DMGEFF_ICE_MAGIC)) {
                ...
                if (this->damageEffect == REDEAD_DMGEFF_FIRE_MAGIC) {
                    ...
                    if (this->actor.params <= REDEAD_TYPE_GIBDO) {
                        ...
                        this->fireTimer = 60;//original was 40;
                    }
                } else {
...
void EnRd_Update(Actor* thisx, PlayState* play) {
    ...
    if(((player->actor.freezeTimer > 0) && ((this->actionFunc == EnRd_AttemptPlayerFreeze) || (this->actionFunc == EnRd_WalkToPlayer)) ) || (this->action == REDEAD_ACTION_GRAB) ){
        SoundEffect_PlayHeld(SOUND_VIOLIN_TENSE, 1.0f,1.1f,1.0,&this->actor.world.pos,1.0f,1000.0f, &this->actor);
    }
