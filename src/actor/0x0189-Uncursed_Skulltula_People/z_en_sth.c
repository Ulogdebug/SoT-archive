...
#include "oot/ovl_En_Sth/ovl_En_Sth.c"
...
static s16 sGetItemIds[6] = {
    GI_RUPEE_GOLD /* 120 */, GI_WALLET_ADULT, GI_HEART_PIECE /*20*/, GI_STONE_OF_AGONY, GI_WALLET_GIANT, GI_TUNIC_ZORA,
};
...
void EnSth_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params == 0) {
        if (gSaveContext.inventory.gsTokens < 120) {
...
void EnSth_SetupAfterObjectLoaded(EnSth* this, PlayState* play) {
    ...
    if (gSaveContext.eventChkInf[EVENTCHKINF_DA_DB_DC_DD_DE_INDEX] & this->eventFlag) {
        if(this->actor.params == 5 && !(CHECK_OWNED_EQUIP(EQUIP_TYPE_TUNIC, EQUIP_INV_TUNIC_ZORA))){
