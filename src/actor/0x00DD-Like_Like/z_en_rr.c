...
void EnRr_SetupRetreat(EnRr* this);
void EnRr_WaitForFalling(EnRr* this, PlayState* play);
...
static DamageTable sDamageTable = {
    ...
    /* Deku stick    */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Slingshot     */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Explosive     */ DMG_ENTRY(2, RR_DMG_NORMAL),
    /* Boomerang     */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Normal arrow  */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Hammer swing  */ DMG_ENTRY(4, RR_DMG_NORMAL),
    /* Hookshot      */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Kokiri sword  */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Master sword  */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Giant's Knife */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Fire arrow    */ DMG_ENTRY(4, RR_DMG_FIRE),
    /* Ice arrow     */ DMG_ENTRY(0, RR_DMG_ICE),
    ...
    /* Ice magic     */ DMG_ENTRY(0, RR_DMG_ICE),
    /* Light magic   */ DMG_ENTRY(10, RR_DMG_LIGHT_MAGIC),
    /* Shield        */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Mirror Ray    */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Kokiri spin   */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Giant spin    */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Master spin   */ DMG_ENTRY(0, RR_DMG_STUN),
    /* Kokiri jump   */ DMG_ENTRY(0, RR_DMG_NONE),
    /* Giant jump    */ DMG_ENTRY(2, RR_DMG_NORMAL),
    /* Master jump   */ DMG_ENTRY(1, RR_DMG_NORMAL),
    /* Unknown 1     */ DMG_ENTRY(10, RR_DMG_SPRT_ARROW),
    /* Unblockable   */ DMG_ENTRY(4, RR_DMG_NORMAL),
    /* Hammer jump   */ DMG_ENTRY(4, RR_DMG_NORMAL),
...
void EnRr_Init(Actor* thisx, PlayState* play2) {
    ...
    s16 stickToCelling = (this->actor.params & 0xF); // PARAMS : 000*
    this->actor.scale.x = this->actor.scale.z =  0.014f;
    this->actor.gravity = (stickToCelling) ? 0.0f : -0.4f;
    this->actionTimer = 0;
    this->eatenShield = 0;
    this->eatenTunic = 0;
    this->retreat = false;
    ...
    this->actionFunc = (stickToCelling) ? EnRr_WaitForFalling : EnRr_Approach;
    for (i = 0; i < 5; i++) {
...
void EnRr_SetSpeed(EnRr* this, f32 speed) {
    ...
    SoundEffect_PlayOneshot(SOUND_WORM_CRAWL, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,800.0f,&this->actor);
...
void EnRr_WaitForFalling(EnRr* this, PlayState* play) {
    ...
    if ((play->gameplayFrames & 0x5F) == 0) {
        SoundEffect_PlayOneshot(SOUND_WORM_CRAWL, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,1000.0f,&this->actor);
    }
    if(this->actor.xzDistToPlayer < 80.0f){
        this->actor.gravity = -0.4f;
    }
    if(this->actor.bgCheckFlags & BGCHECKFLAG_GROUND){
        SoundEffect_PlayOneshot(SOUND_ENEMY_GARO_ON_GROUND, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,800.0f,&this->actor);
...
void EnRr_SetupReach(EnRr* this, PlayState* play) {
    ...
    this->segMoveRate = 0.0f;//original was 0.0f
    for (i = 0; i < 5; i++) {
    ...
    SoundEffect_PlayOneshot(SOUND_WORM_SQUEZE, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,1000.0f,&this->actor);
...
void EnRr_SetupNeutral(EnRr* this) {
    ...
    if (this->retreat) {
        this->actionTimer = 45; //original was 100
        this->actionFunc = EnRr_SetupRetreat;
    } else  {
        this->actionTimer = 20;
...
void EnRr_SetupGrabPlayer(EnRr* this, Player* player) {
    ...
    this->grabTimer = 150; //the timer decides when your gears is comsumed
...
void EnRr_SetupReleasePlayer(EnRr* this, PlayState* play) {
    ...
    tunic = 0;
    shield = 0;
    if ((CUR_EQUIP_VALUE(EQUIP_TYPE_SHIELD) != EQUIP_VALUE_SHIELD_MIRROR)&&(this->grabTimer <= 0)) {
    ...
    if (this->actor.colorFilterTimer == 0) {
        //this->actionFunc = EnRr_Approach;
        this->actionFunc = EnRr_SetupReach;//incompatible pointer type
        Audio_PlayActorSfx2(&this->actor, NA_SE_EN_LIKE_THROW);
        SoundEffect_PlayOneshot(SOUND_WORM_SQUISH, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,1000.0f,&this->actor);
...
void EnRr_SetupDamage(EnRr* this) {
    ...
    SoundEffect_PlayOneshot(SOUND_WORM_DAMAGE, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,1000.0f,&this->actor);
    Audio_PlayActorSfx2(&this->actor, NA_SE_EV_BUYODOOR_OPEN);
...
void EnRr_SetupDeath(EnRr* this) {
    ...
    SoundEffect_PlayOneshot(SOUND_WORM_DIE, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,1000.0f,&this->actor);
    this->actor.flags &= ~ACTOR_FLAG_0;
}
...
void EnRr_CollisionCheck(EnRr* this, PlayState* play) {
    ...
    if ((this->collider2.base.acFlags & AC_HIT) ) {
        if((this->collider2.info.acHitInfo->toucher.dmgFlags & DMG_HAMMER_JUMP) ){
            this->collider1.base.acFlags |= AC_HIT;
            this->actor.colChkInfo.damageEffect = 15;
            this->actor.colChkInfo.damage = 4;
        }else if((this->collider2.info.acHitInfo->toucher.dmgFlags & DMG_HAMMER_SWING)){
            this->collider1.base.acFlags |= AC_HIT;
    ...
    if (this->collider1.base.acFlags & AC_HIT) {
        ...
        hitPos.z = this->collider1.info.bumper.hitPos.z;
            ...
            if (this->actor.colChkInfo.damageEffect == 0) {
                Audio_PlaySfxGeneral(NA_SE_EV_BUYODOOR_OPEN, &this->actor.world.pos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            }
            CollisionCheck_GreenBlood(play, NULL, &hitPos); 
            switch (this->actor.colChkInfo.damageEffect) {
                ...
                case RR_DMG_STUN: // Boomerang and Hookshot
                    ...
                    Actor_SetColorFilter(&this->actor, -0x8000, 0xFF, 0x2000, 0x35);//the original were 0xFF  0x2000 0x50
...
void EnRr_Approach(EnRr* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xA, 0x220, 0);// the original were 0xA, 0x1F4, 0  */  
                                                                                              //s16* pValue, s16 target, s16 scale, s16 step, s16 minStep
    this->actor.world.rot.y = this->actor.shape.rot.y;
    if((!(player->stateFlags2 & PLAYER_STATE2_7))){
        if ((this->actionTimer == 0) && (this->actor.xzDistToPlayer < 160.0f) /*&& (this->actor.xzDistToPlayer >= 120.0f)*/) {
            EnRr_SetupReach(this,play);
        } else if ((this->actor.xzDistToPlayer < 500.0f) && (this->actor.speedXZ == 0.0f)) {
            EnRr_SetSpeed(this, 5.0f);//original 2.0f
        }
    }else if(this->actor.xzDistToPlayer < 160.0f){
        EnRr_SetupRetreat(this);
    }
}

void EnRr_Reach(EnRr* this, PlayState* play) {
    Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xA, 0x3B6, 0); // the original were 0xA, 0x1F4, 0  */
    Math_SmoothStepToF(&this->actor.speedXZ, 0.0f, 0.2f, 1.0f, 0.0f);  
    this->actor.world.rot.y = this->actor.shape.rot.y;
    switch (this->reachState) {
        ...
        case REACH_GAPE:
            if (this->actionTimer == 0) {
                if ((this->actor.xzDistToPlayer >  120.0f) && (this->actor.xzDistToPlayer <= 180.0f))
                {
                    this->actionTimer = Rand_S16Offset(8, 1);
                }
                else if (this->actor.xzDistToPlayer <= 120.0f) {
                    this->actionTimer = Rand_S16Offset(5, 0);//the original was 20
                }
                else if ((this->actor.xzDistToPlayer > 180.0f)&& (this->actor.xzDistToPlayer <=250.0f)) {
                    this->actionTimer = Rand_S16Offset(20, 15);
                }
...
void EnRr_GrabPlayer(EnRr* this, PlayState* play) {
    ...
    if(this->grabTimer == 80){
         Message_StartTextbox(play, 0x01B1, NULL);
    }
    ...
    if ((this->frameCount % 4) == 0) {//the original was 8
    ...
    if(this->SuckTimer == 8 ){
    ...
    else if(this->SuckTimer == 0) {
        if (rand01 < 0.3f ){
            play->damagePlayer(play, -12);
            func_8002F7DC(&player->actor, NA_SE_VO_LI_FREEZE + player->ageProperties->unk_92);//Poor Link's fiercely damaged scream
        }else{
       ...
       // Audio_PlayActorSfx2(&this->actor, NA_SE_EN_GOMA_CRY2);
        SoundEffect_PlayOneshot(SOUND_WORM_SQUISH, 0.75f, 1.0f,&this->actor.world.pos,DEFAULT_REVERB,1000.0f,&this->actor);
        // Audio_PlayActorSfx2(&this->actor, NA_SE_VO_LI_DAMAGE_S);
        this->SuckTimer = 15;
...
void EnRr_Retreat(EnRr* this, PlayState* play) {
    ...
    if (this->actionTimer == 0) {
        ...
        if (this->actor.speedXZ == 0.0f) {
            EnRr_SetSpeed(this, 5.0f);//original speed 2.0f
        }
    }
}
...
void EnRr_Update(Actor* thisx, PlayState* play) {
    ...
    EnRr* this = (EnRr*)thisx;
    s32 i;
    Debug_Text(255,0,0,3,3,"huntMode: %d",this->huntMode);
    this->frameCount++;
    if (!this->stopScroll) {
        this->scrollTimer++;
    }
