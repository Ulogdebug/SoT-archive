...
void EnPoDesert_WaitForPlayer(EnPoDesert* this, PlayState* play) {
    ...
    f32 dis = 200.0f;
    if(play->sceneId == 0x6B){
        dis = 350.0f;
    }
    if (this->actor.xzDistToPlayer < dis && (this->currentPathPoint != 2 || play->actorCtx.lensActive)) {
        if (this->currentPathPoint == 2) {
            ...
            if(play->sceneId == 0x6B){
                 Message_StartTextbox(play, 0x101C, NULL);
            }else{
...
void EnPoDesert_MoveToNextPoint(EnPoDesert* this, PlayState* play) {
    ...
    if(play->sceneId == 0x6B){
        this->actor.speedXZ = sinf(this->speedModifier * (M_PI / 32.0f)) * 2.5f + 10.5f;
    }else{
