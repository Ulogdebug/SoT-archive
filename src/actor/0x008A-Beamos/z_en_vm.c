...
void EnVm_Lightning(EnVm* this, PlayState* play);
void EnVm_BigLightning(EnVm* this, PlayState* play);
...
static ColliderCylinderInit sCylinderInit = {
    {
        COLTYPE_METAL,                                          //original were COLTYPE_METAL,                                                    
        AT_ON | AT_TYPE_ENEMY,                                          //original were  AT_NONE,                       
        AC_ON | AC_HARD | AC_TYPE_PLAYER,                       //original were  AC_ON | AC_HARD | AC_TYPE_PLAYER,
        OC1_ON | OC1_TYPE_ALL,
        OC2_TYPE_1,         
        COLSHAPE_CYLINDER,                                       
    },
    {
        ELEMTYPE_UNK0,
        { 0xFFCFFFFF, 0x03, 0x10 },   //original were { 0x00000000, 0x00, 0x00 }, 
        { 0xFFCFFFFF, 0x00, 0x00 },
        TOUCH_ON | TOUCH_SFX_NONE, //original were TOUCH_ON
        BUMP_ON,
        OCELEM_ON,
    },
    { 20, 48, 0, { 0, 0, 0 } },
...
static ColliderQuadInit sQuadInit1 = {
    ...
    {
        ELEMTYPE_UNK0,
        { 0xFFCFFFFF, 0x00, 0x04 }, //original were 0xFFCFFFFF, 0x00, 0x10
...
static Vec3f D_80B2EB34 = { 2000.0f, 1700.0f, 2000.0f };//

static Vec3f D_80B2EB40 = { 2000.0f, -1700.0f, 2000.0f };//

static Vec3f D_80B2EB4C = { -2000.0f, 1700.0f, 1500.0f };//

static Vec3f D_80B2EB58 = { -2000.0f, -1700.0f, 1500.0f };//
...
void EnVm_Init(Actor* thisx, PlayState* play) {
    ...
    //this->beamSightRange = ((thisx->params >> 8)*100 + );//original was  (thisx->params >> 8) * 40.0f
    thisx->params &= 0xFF;
    ...
    if (thisx->params == BEAMOS_LARGE) {
        ...
        this->beamSightRange = ((thisx->params >> 8) * 100 + 500);
    } else {
        thisx->colChkInfo.health = 1;
        Actor_SetScale(thisx, 0.01f);
        this->colliderQuad1.info.toucher.damage = 0x08;
        this->beamSightRange = ((thisx->params >> 8) * 100 + 300);
    }
...
void EnVm_Wait(EnVm* this, PlayState* play) {
    ...
    switch (this->unk_25E) {
        case 0:
            ...
            if (this->actor.xzDistToPlayer <= this->beamSightRange && ABS(headRot) <= 0x2710/* && pitch >= 0xE38 */ &&
                this->actor.yDistToPlayer <= 80.0f && this->actor.yDistToPlayer >= -160.0f) {// Original were 80.0f , -160.0f
    ...
    Math_SmoothStepToS(&this->headRotY, this->actor.yawTowardsPlayer - this->actor.shape.rot.y, 1, 0x740, 0);
    ...
    if (this->unk_260 == 2) {
        ...
        if (this->beamRot.x < 0x001) {//original were 0xAAA
...
void EnVm_SetupAttack(EnVm* this) {
    ...
    Audio_PlayActorSfx2(&this->actor, NA_SE_IT_SHIELD_REFLECT_MG2);
...
void EnVm_Attack(EnVm* this, PlayState* play) {
    ...
    if (this->beamRot.x < 0x001 || this->timer == 0 ) {//original were 0xAAA
        Math_SmoothStepToF(&this->beamScale.x, 0.0f, 1.0f, 0.03f, 0.0f);
        this->unk_260 = 0;

        if (this->beamScale.x == 0.0f || (this->actor.xzDistToPlayer >(this->beamSightRange) * 1.5) || (this->actor.xzDistToPlayer > 1000)) {
...
void EnVm_SetupStun(EnVm* this) {
    ...
    Audio_PlayActorSfx2(&this->actor, NA_SE_SY_FSEL_ERROR);
...
void EnVm_CheckHealth(EnVm* this, PlayState* play) {
    EnBom* bomb;
    s16 colliderDMG = this->colliderCylinder.info.acHitInfo->toucher.dmgFlags;
    s16 gotValidAttacked = 0;
    if (this->colliderQuad2.base.acFlags & AC_HIT) {
        this->colliderQuad2.base.acFlags &= ~AC_HIT;
        if(this->colliderQuad2.info.acHitInfo->toucher.dmgFlags & DMG_EXPLOSIVE){
            if(this->stunFlag){
                gotValidAttacked = 1 ;
            }else{
                EnVm_BigLightning(this, play);
                return;
            }
        }else{
        ...
        if ((this->colliderCylinder.info.acHitInfo->toucher.dmgFlags & DMG_EXPLOSIVE)) {
            if (this->stunFlag){
                gotValidAttacked = 1;
            }
            else {
        ...
        if(this->colliderCylinder.info.acHitInfo->toucher.dmgFlags & DMG_HAMMER){
            if (this->stunFlag){
                gotValidAttacked = 1;
            }else{
                Actor_SetColorFilter(&this->actor, 0, 0xFF, 0, 8);
        ...
        }else if((!this->stunFlag) && (this->colliderCylinder.info.acHitInfo->toucher.dmgFlags & DMG_SLASH) ){
            EnVm_BigLightning(this, play);
    ...
    if (this->actor.colChkInfo.health <= 0) {
        Actor_SetColorFilter(&this->actor, 0x4000, 0xFF, 0, 64);
        bomb = (EnBom*)Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOM, this->actor.world.pos.x,
...
void EnVm_Lightning(EnVm* this, PlayState* play)
{
    ...
    static Color_RGBA8 envColor = { 255, 255, 0, 255 };
    s32 i;
    Vec3f effectPos;
    s16 effectYaw;
    s16 effectXZ;
    ...
    for (i = 0; i < 4; i++) {
        if (!((this->timer + (i << 1)) % 4)) {
            ...
            effectPos.y = (Rand_ZeroOne() * 5.0f) + this->actor.world.pos.y + 40.5f;
            effectPos.z = Rand_CenteredFloat(5.0f) + this->actor.world.pos.z;
            EffectSsLightning_Spawn(play, &effectPos, &primColor, &envColor, effectXZ, effectYaw, 3, random);
        }
    }
...
void EnVm_BigLightning(EnVm* this, PlayState* play)
{
    ...
    for (i = 0; i < 4; i++) {
        if (!((this->timer + (i << 1)) % 4)) {
            effectYaw = (s16)Rand_CenteredFloat(12288.0f) + (i * 0x4000) + 0x3000;
            effectPos.x = Rand_CenteredFloat(5.0f) + this->actor.world.pos.x;
            effectPos.y = (Rand_ZeroOne() * 5.0f) + this->actor.world.pos.y + 40.5f;
            effectPos.z = Rand_CenteredFloat(5.0f) + this->actor.world.pos.z;
            EffectSsLightning_Spawn(play, &effectPos, &primColor, &envColor, effectXZ, effectYaw, 3, random);
    ...
    Audio_PlayActorSfx2(&this->actor, NA_SE_IT_SHIELD_CHARGE_LV3 - SFX_FLAG );
...
void EnVm_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->stunFlag != 1) {
        ...
        EnVm_Lightning(this, play);

    }

    if ((this->colliderCylinder.base.atFlags & AT_ON) && (this->actor.colChkInfo.health != 0) && (this->actor.colorFilterTimer == 0) && (this->stunFlag != 1)) {
...
void EnVm_Draw(Actor* thisx, PlayState* play2) {
    ...
    if (this->unk_260 >= 3) {
        ...
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 50, 50, 168);
