...
void En_Hy_BarBossSetTextID(EnHy* this);
void EnHy_BarBossTradeSetup(EnHy* this, PlayState* play);
void EnHy_BarBossDialogEnd(EnHy* this, PlayState* play);
void EnHy_BarBossGiveKeySetup(EnHy* this, PlayState* play);
void EnHy_BarBossTrade(EnHy* this, PlayState* play);
void EnHy_BarBossDialogJudge(EnHy* this, PlayState* play);
...
static s16 dancingGirlRandomDialgoues[] = {0x7014,0x70B5,0x70A4,0x70B4,0x70BA,0x70C2,0x70B0,0x70B1,0x70B2,0x70C3,0x70C5,0x70C1,0x70C1};
...
u16 En_Hy_dialogueJudge(PlayState* play, Actor* thisx) {
    Player* player = GET_PLAYER(play);
    EnHy* this = (EnHy*)thisx;
    u16 textId = Text_GetFaceReaction(play, (this->actor.params & 0x7F) + 37);
    s16 i = Rand_S16Offset(0,12);
    if (textId != 0) {
    ...
    switch (this->actor.params & 0x7F) {
        case ENHY_TYPE_AOB:
            ...
            /** } else**/ if (gSaveContext.dogIsLost) {
                ...
                if (followingDog != 0) {
                    ...
                    return (GET_INFTABLE(INFTABLE_CB) ? 0x508C : 0x709D);
                }
            } else {
                return 0x70A0;
            }
        case ENHY_TYPE_COB: //fat woman in auction
            if (CustomFlag_Get(HAS_SOLD)) {
                return GET_INFTABLE(INFTABLE_C1) ? 0x7045 : 0x703F;
            } else {
                return GET_INFTABLE(INFTABLE_C0) ? 0x7017 : 0x7016;
            }
        case ENHY_TYPE_AHG_2: //Beard man
            // if (play->sceneId == SCENE_KAKARIKO) {
            //     return 0x5086;
            // } else if (play->sceneId == SCENE_SPOT01) {
            //     return 0x5085;
            if (!CustomFlag_Get(HAS_SOLD)) {
                return GET_INFTABLE(INFTABLE_C3) ? 0x7047 : 0x701A;
            }else {
                return GET_INFTABLE(INFTABLE_C2) ? 0x701C : 0x701B;
            } /*else if (GET_EVENTCHKINF(EVENTCHKINF_14)) {
                return 0x701A;
            } else if (GET_EVENTCHKINF(EVENTCHKINF_10)) {
                return 0x701A;//0x701B;
        ...
        case ENHY_TYPE_CNE_11:
            return dancingGirlRandomDialgoues[i];
        ...
        case ENHY_TYPE_AHG_13:
            return GET_EVENTCHKINF(EVENTCHKINF_10) ?0x7056:0x7055;
        ...
        case ENHY_TYPE_BOJ_16:
            return 0x700E;
        case ENHY_TYPE_AHG_17: //Monster Gallery Owner          
...
void En_Hy_DialogueUpdate(EnHy* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    s16 phi_a3;
    switch (this->actor.params & 0x7F) {
        case ENHY_TYPE_BOJ_3:
  ...
  if (func_800343CC(play, &this->actor, &this->unk_1E8.unk_00, this->unkRange, En_Hy_dialogueJudge, func_80A70058)) {
         func_80A70834(this, play);//used for begger
    }
}

void En_Hy_BarBossSetTextID(EnHy* this) {
    if (GET_INFTABLE(INFTABLE_B6)) {
        this->actor.textId = 0x701A;
    } else {
        this->actor.textId = 0x0401;//firts talk
    }
...
void EnHy_BarBossDialogJudge(EnHy* this, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, play)) {
        ...
        this->actionFunc = EnHy_BarBossTradeSetup;
    }
    ...
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE && Message_ShouldAdvance(play)) {
        ...
        case 0: // yes
            if (gSaveContext.rupees < 50) {
                Message_ContinueTextbox(play, 0xC8); // no you don' t have enough money
                this->actionFunc = EnHy_BarBossDialogEnd;
            }
            else {               
                func_800849EC(play);
                Rupees_ChangeBy(-50);                
                this->actionFunc = EnHy_BarBossGiveKeySetup;
            }
            break;
        case 1: // no
...
void EnHy_BarBossGiveKeySetup(EnHy* this, PlayState* play) {
    if (Actor_HasParent(&this->actor, play)) {
        ...
        this->actionFunc = EnHy_BarBossTrade;
    } else {
        func_800849EC(play);
    }
}
...
void EnHy_BarBossTrade(EnHy* this, PlayState* play){
    func_8002F434(&this->actor, play, GI_KEY_SMALL, 130.0f, 100.0f);
...
s32 EnHy_ShouldSpawn(EnHy* this, PlayState* play) {
    switch (play->sceneId) {
        ...
        case SCENE_KAKARIKO:
            if ((this->actor.params & 0x7F) == ENHY_TYPE_AOB) {
                return !LINK_IS_ADULT ? true : true;
...
void EnHy_Update(Actor* thisx, PlayState* play) {
    ...
    En_Hy_DialogueUpdate(this, play);
