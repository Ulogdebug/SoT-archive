...
void EnTorch2_Init(Actor* thisx, PlayState* play2) {
    ...
    if (Flags_GetSwitch(play, 0x1)) {
         Actor_Spawn(&play->actorCtx, play, ACTOR_DOOR_WARP1,  -9741,  -1216,
                     -1684, 0, 0, 0, 0x8);
        Actor_Kill(thisx);
        return;
    }
...
void EnTorch2_Update(Actor* thisx, PlayState* play2) {
    ...
    switch (sActionState) {
        case ENTORCH2_WAIT:
            ...
            this->actor.world.pos.x = sSpawnPoint.x;
            this->actor.world.pos.z = sSpawnPoint.z;
                    ...
                    if (func_800354B4(play, &this->actor, 120.0f, 0x7FFF, 0x7FFF, this->actor.world.rot.y)) {
                        if ((player->meleeWeaponAnimation == PLAYER_MWA_STAB_1H) &&
                            ...
                            if ((this->meleeWeaponState == 0) && (sCounterState == 0) &&
                                ...
                                (this->actor.xzDistToPlayer <= 85.0f) && Actor_IsTargeted(play, &this->actor) && (Actor_IsFacingPlayer(&this->actor, 0x4000))) {
                                ...
                                sHoldShieldTimer = 0;
                                input->cur.button = BTN_A;
                            } else if((this->actor.xzDistToPlayer <= 30.0f)&&(Actor_IsFacingPlayer(&this->actor, 0x4000))){
                        ...
                        if ((90.0f >= this->actor.xzDistToPlayer) && (this->actor.xzDistToPlayer > 70.0f) &&
                            ...
                            if (!EnTorch2_SwingSword(play, input, this) && (this->meleeWeaponState == 0) &&
                                (sCounterState == 0) && (Actor_IsFacingPlayer(&this->actor, 0x4000))) {
                                input->cur.button = BTN_R;
                            }
                            else if ( !Actor_IsFacingPlayer(&this->actor, 0x4000) && (this->actor.xzDistToPlayer <= 50.0f)) {
                                EnTorch2_Backflip(this, input, &this->actor);
                            }
