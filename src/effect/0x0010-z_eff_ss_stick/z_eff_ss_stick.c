...
u32 EffectSsStick_Init(PlayState* play, u32 index, EffectSs* this, void* initParamsx) {
    ...
    StickDrawInfo drawInfo[] = {
        { OBJECT_LINK_BOY, gPlayasAdult_DlBlade3 }, // adult, broken sword
        { OBJECT_LINK_CHILD, gPlayasAdult_DlBlade3 }, // child, broken stick
    };

    Player* player = GET_PLAYER(play);

    if (player->heldItemActionParam == PLAYER_AP_STICK){
        drawInfo[0].displayList = gPlayasAdult_DlDekuStickBrokenHeld;
        //drawInfo[1].displayList = gPlayasChild_DlDekuStickBroken;
    }else{
        if (LINK_IS_ADULT){
            drawInfo[0].displayList = gPlayasAdult_DlBlade3;
        }else{
